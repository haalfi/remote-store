...
post-rename properties read does not re-expose `Content-MD5` (the sibling `write` populates
`digest` on the same account). For a guaranteed digest regardless of method or backend, use the
