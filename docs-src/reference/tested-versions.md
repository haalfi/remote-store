...
Each `[<extra>]` declares a floor in `pyproject.toml` and in most cases deliberately no ceiling (the `arrow` and `sql-query` extras carry a `pyarrow<25` ceiling — see the comment on `[project.optional-dependencies]` in `pyproject.toml`). The drift guard (`.github/workflows/drift-guard.yml`, ID-182) records the last known-good resolution per extra in `infra/drift-locks/` and re-resolves weekly against the latest available versions (including pre-releases) to surface silent transitive upgrades before they reach users.
