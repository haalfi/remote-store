"""Tests for Store — derived from docs/specs/store.md. Integration tests via local backend."""
...
def test_store_construction() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        backend = LocalBackend(root=tmp)
...
def test_invalid_path_rejected(store: Store) -> None:
    with pytest.raises(InvalidPath):
        store.read("../escape")


@pytest.mark.spec("STORE-002")
def test_empty_path_rejected(store: Store) -> None:
    with pytest.raises(InvalidPath):
        store.read("")
...
def test_root_path_prepended(store: Store) -> None:
    store.write("hello.txt", b"hi")
    # The file should be at data/hello.txt in the backend
...
def test_write_and_read(store: Store) -> None:
    store.write("test.txt", b"content")
    assert store.read_bytes("test.txt") == b"content"


@pytest.mark.spec("STORE-004")
def test_read_stream(store: Store) -> None:
...
def test_supports(store: Store) -> None:
...
def test_exists(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_is_file_is_folder(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_write_overwrite(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_write_atomic(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_delete_missing_ok(store: Store) -> None:
    store.delete("nonexistent.txt", missing_ok=True)


@pytest.mark.spec("STORE-008")
def test_delete_not_found(store: Store) -> None:
    with pytest.raises(NotFound):
        store.delete("nonexistent.txt")


@pytest.mark.spec("STORE-008")
def test_delete_folder(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_list_files(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_list_files_recursive(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_get_file_info(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_get_folder_info(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_move(store: Store) -> None:
...
@pytest.mark.spec("STORE-008")
def test_copy(store: Store) -> None:
