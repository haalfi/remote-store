...
def test_every_async_fixture_matches_an_async_extended_scope() -> None:
    ...
    for f in all_fixtures():
        ...
        # every test that issues a request, so they are intentionally excluded
        # from the async-extended scopes (a scope built around them aborts the
        # gremlins baseline with 'No data was collected', exit 3). Exempt them
        # here in lockstep with ``mutate_scopes._async_extended_runnable``; the
        # exemption self-lifts once cassettes land, as it has for graph.
