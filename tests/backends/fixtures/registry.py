...
class BackendFixture:
    ...
    **every** mode. It stays visible to ``all_fixtures()``, so it still opts
    into cassette-directory routing, name aliasing, the scrub config, and the
    leak audit via its ``cassette_profile`` (REC-007), and the recorder's ``-k``
    ...
    * ``single_connection`` — one instance per thread; concurrent ops on one
    ...
    cassette-directory routing, name aliasing, the scrub config, and the leak
    audit. Non-HTTP fixtures (and HTTP fixtures without a cassette tier, e.g.
    Azurite) leave it ``None`` and are invisible to cassette routing. The
    missing-cassette skip is *not* on that list: it keys on an unplayable
    request against an absent cassette file and so needs no registration.
    """
