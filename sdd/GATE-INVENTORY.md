...
| `scripts/check_backend_order.py` | the canonical backend order pinned in CONTRIBUTING.md ↔ every backend enumeration in README.md, docs-src/, context7.json and the conda recipe | cross-surface | `all`, `docs-gate`, `lint` | gating |
...
| `scripts/docs/check_links.py` | every Markdown link and context7 manifest entry ↔ the on-disk files and built docs-site pages they name | explanation | `all`, `docs-gate` | gating |
...
| `scripts/gen_features.py` | docs-src/_data/graph/graph.json and pyproject.toml's extras ↔ the generated sections of FEATURES.md | realization ↔ explanation | `all`, `preflight` | gating |
