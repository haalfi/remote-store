...
- **(b)** The cross-protocol DEPTH-003 *result* invariant is consolidated onto pre-existing tests rather than duplicated into a new conformance file: `tests/backends/conformance/test_listing.py::TestListFilesCompleteness` already parametrizes `list_files(max_depth=…)` over the full fixture registry, and `tests/backends/azure/test_config.py::test_list_files_max_depth` (BUG-155) already exercises it behaviourally for Azure (Azurite, Stage 2). `@pytest.mark.spec("DEPTH-003")` was stacked onto both. *(Update, BK-255: the Azure-specific `test_config.py::test_list_files_max_depth` was later removed as redundant with the registry-parametrised conformance owner; the Azure behavioural DEPTH-003 invariant is now owned solely by `conformance/test_listing.py::TestListFilesCompleteness::test_list_files_recursive_max_depth` over the Azurite fixture, where the BUG-155 breadcrumb moved to the test docstring.)*
