...
deprecated. Users can tune any of it via engine kwargs when passing a pre-built
engine.
...
**A per-thread pool is not by itself isolating, and `QueuePool` is not by itself
