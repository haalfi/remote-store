...
3. **Real-but-local.** Real backend code paths against a local
   fixture, library, or Docker service. Examples include
   `LocalBackend`, `MemoryBackend`, SQLite, Azurite, MinIO, and
   Dockerised SFTP.
4. **Real-but-live.** Real backend code paths against a live cloud
...
1. **Stage 1.** Repo only. Zero cost. No restriction on volume,
   amount, or frequency. Default for `pytest` and CI.
2. **Stage 2.** Requires Docker. Zero cost. No restriction on volume,
   amount, or frequency. Default for `hatch run test` and PR CI.
...
class BackendFixture:
    ...
    factory: Callable[[], Backend]  # produces a fresh isolated instance
    stage: int                      # 1, 2, or 3 per TEST-001
    kind: str                       # "pure", "mock", "real-local", or "real-live"
    capabilities: frozenset[Capability]
    is_async: bool
    cleanup: Callable[[Backend], None] | None = None
```
...
are included. Default is stage 2 for `hatch run test` and stage 1 for
any context where Docker is unavailable. Each stage includes all
lower stages.

| Stage flag | Fixtures included | Required environment |
|---|---|---|
| `--stage=1` | pure plus replay (when cassettes present) | none |
| `--stage=2` (default) | Stage 1 plus Docker fixtures | Docker daemon reachable |
| `--stage=3` | Stage 1 plus Stage 2 plus live fixtures | per-backend live env vars (e.g. `RS_TEST_LIVE_HNS=1`) |

**Postcondition:** `pytest` with no flags runs Stage 1 plus Stage 2
by default. A developer without Docker can opt down with
`--stage=1`. Stage 3 is never implicit. Missing env vars cause Stage
3 fixtures to skip loudly with a fixture-level `pytest.skip(...)`
referencing the missing variable.
...
**Rationale:** [ADR-0028](../adrs/0028-testing-architecture-kind-stage-replay.md)
§ HTTP cassette and replay scoped to HTTP backends.
