...
**Invariant:** Whether the storage account has Hierarchical Namespace (HNS) enabled is a **mandatory, explicit** constructor and config input: `AzureBackend(..., hns: bool)`. There is no default and no runtime auto-detection. A backend constructed without `hns`, or with a non-`bool` value (e.g. the string `"false"` produced by config env-var resolution), raises `ValueError` at construction time (fail loud — the declaration must be a real boolean, never a truthy/falsy proxy that could be silently coerced, and the account's nature is never silently inferred). The declared value is stored as an immutable attribute and governs every adaptive code path below; because it is a constant, no operation can observe a different HNS state than the one declared (no probe, no cache, no per-operation snapshot).
...
**Postconditions:** The `BackendUnavailable` returned for the `ServiceRequestError` / `ServiceResponseError` row always carries a non-empty message, per [ERR-009](005-error-model.md). The driver's own text is kept whenever it has any; only a signal that carries none is given a synthesised message, naming which side of the exchange failed and the SDK exception class.

**No row of the table can be empty, but not all for the same reason**, and the distinction is worth keeping because only one of them is this classifier's doing: the first four rows are composed here, from the path and — for the throttle and gateway row — the status, so they cannot be empty whatever the SDK passes in. The last row passes `str(exc)` through unchanged and composes nothing; it is non-empty because `HttpResponseError.__init__` substitutes `Operation returned an invalid status '<reason>'` for a falsy message, which is a guarantee of the SDK's and not of this spec's.

**One outcome of the classifier is not a row of the table, and is not covered.** An exception matching none of the rows — not an `HttpResponseError` and not one of the typed subclasses — falls through to the base `RemoteStoreError`, which passes `str(exc)` through and *can* be empty: a bare `RuntimeError` renders as `" | path='delivery.csv' | backend='azure'"`. That is deliberate scope, not an oversight. It is the same construction the base class carries on four other backends, the decision is whether to synthesise or to classify, and it is answered once for all of them rather than here; the backlog item for it is named in the register entry that closed this clause's own defect. **Read the guarantee above as covering the mapped outcomes, not the function.**
...
**Why the message clause is here and not only in the error model, stated once:** `AzureError.__init__` sets `self.message = str(message)`, so a wrap of an argument-less driver exception stringifies empty and `BackendUnavailable(str(exc))` renders as `" | path='delivery.csv' | backend='azure'"` — the promised type, saying nothing. **Both transports supply such an input, by different routes.** The async (aiohttp) transport's `except asyncio.TimeoutError` arms wrap what they caught as `ServiceResponseTimeoutError`, and a fired timeout carries no arguments. The sync (requests) transport wraps `requests.exceptions.ConnectTimeout` as `ServiceRequestTimeoutError` and `ReadTimeout` as `ServiceResponseTimeoutError`, and both of those stringify empty when constructed without arguments. Neither transport is quoted by line number and no count of arms is given: it is a third-party file's shape, it moves between SDK releases, and the revision of this work that pinned the shape in four artifacts falsified three of them in a single change. This paragraph is the one home; the helper, its tests and the register entry point here rather than restating it.

The invariant belongs on the row that can breach it. Note the asymmetry the two transports create: a blank `ServiceResponseTimeoutError` is reachable on both, while a blank `ServiceRequestTimeoutError` is a sync-transport shape — the async transport builds that class only from an aiohttp error that always carries the host it could not reach.
