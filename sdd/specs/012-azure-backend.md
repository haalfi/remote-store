...
**Why the message clause is here and not only in the error model:** `AzureError.__init__` sets `self.message = str(message)`, and the aiohttp transport wraps a bare `asyncio.TimeoutError` — which carries no arguments — as `ServiceResponseTimeoutError(err, error=err)`. `BackendUnavailable(str(exc))` on that input renders as `" | path='delivery.csv' | backend='azure'"`: the promised type, saying nothing. The invariant belongs on the row that can breach it.
