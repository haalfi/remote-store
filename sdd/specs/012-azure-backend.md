...
**Neither shipped transport supplies such an input through this backend's own options**, which was established by driving both rather than by reading either, and is the opposite of what two earlier revisions of this clause claimed in turn. The async (aiohttp) transport builds its own per-request `ClientTimeout(sock_connect=…, sock_read=…)` and never sets `total`, so what its `except asyncio.TimeoutError` arms catch is aiohttp's `ServerTimeoutError`, which carries text; and it builds `ServiceRequestTimeoutError` only from an aiohttp error naming the host it could not reach. The sync (requests) transport wraps `requests.exceptions.ConnectTimeout` and `ReadTimeout`, and `requests` raises both only with the underlying `urllib3` error attached, which is itself always constructed with a formatted message. That those classes stringify empty when constructed *without arguments* is a fact about the classes, not about what the transports supply — the distinction this spec elsewhere insists on, and the one both earlier revisions elided.

**The guarantee is therefore about the classifier's output, not about a reachable trigger**, which is what ERR-009 asks for. A caller supplying their own transport or session can deliver the empty shape, and a future SDK release can begin to; neither is required for the postcondition to be worth stating, and a mapping that answers correctly only for inputs someone happened to enumerate is the weaker contract. No transport is quoted by line number and no count of arms is given: this is third-party shape that moves between releases, and the revision that pinned it in four artifacts falsified three of them in one change.

This paragraph is the one home. The helper, its tests and the register entry point here rather than restating it; the invariant itself belongs on the row that can breach it.
