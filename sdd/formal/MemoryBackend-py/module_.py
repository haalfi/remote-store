...
class MemoryBackend(Backend):
    ...
    def ctor__(self):
        ...
        (self)._capabilities = _dafny.Set({Capability_CapRead(), Capability_CapWrite(), Capability_CapDelete(), Capability_CapList(), Capability_CapMove(), Capability_CapCopy(), Capability_CapAtomicWrite(), Capability_CapMetadata(), Capability_CapSeekableRead()})
        (self).fs = _dafny.Map({})
...
class Capability:
    ...
    def AllSingletonConstructors(cls):
        return [Capability_CapRead(), Capability_CapWrite(), Capability_CapDelete(), Capability_CapList(), Capability_CapMove(), Capability_CapCopy(), Capability_CapAtomicWrite(), Capability_CapMetadata(), Capability_CapGlob(), Capability_CapSeekableRead()]
    @classmethod
    def default(cls, ):
...
class FileInfo:
    @classmethod
    def default(cls, ):
        return lambda: FileInfo_FileInfo(Path.default(), _dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "")), int(0))
    def __ne__(self, __o: object) -> bool:
        return not self.__eq__(__o)
    @property
...
class FileInfo_FileInfo(FileInfo, NamedTuple('FileInfo', [('path', Any), ('name', Any), ('size', Any)])):
    ...
    def __eq__(self, __o: object) -> bool:
        return isinstance(__o, FileInfo_FileInfo) and self.path == __o.path and self.name == __o.name and self.size == __o.size
    def __hash__(self) -> int:
        return super().__hash__()
