...
| `GetFolderInfo: IsDir → Ok + aggregates` | `test_get_folder_info`, `test_get_folder_info_file_count_and_total_size`, `test_get_folder_info_counts_recursive_children` | `get_folder_info(dir)` → success, `file_count` and `total_size` verified |
