...
- [x] **BUG-275 — A permission-denied SFTP failure raises `PermissionDenied` for one of the two errnos its callers name**
  ...
  **Each subset drew a boundary a caller cannot see**, which is the same defect
  ...
  which no subset has to be argued — which is also why no site count is quoted
  ...
  **Every `Raises:` block now states one rule.** All fifteen naming a permission
  errno say `EACCES` or `EPERM` — `check_health`'s included, which this PR
  rewrote too, adding only that the denial need not be the server's. Derived by
  an AST walk over `_sftp.py` collecting each `Raises:` section that names a
  permission errno: 15, of which the 14 sharing the wording `PermissionDenied:
  If the server denies access` are the ones widened from `(``EACCES``)`, with
  `check_health`'s worded differently. **Two earlier revisions of this sentence
  were wrong in the way it warns about.** The first split the fourteen into six
  it changed and eight it left and claimed the eight "stat no target", false for
  all eight. The second counted the wording string instead of the errnos, got 15
  where that string now matches 14, and called `check_health`'s block errno-less
  and unchanged when the closing commit had already given it both errnos. Both
  are the unrun derivation [CLAUDE.md principle 9](../CLAUDE.md#principles) is
  about; the second survived a round because re-reading the sentence cannot
  catch it and only opening `_sftp.py` can.
  ...
  **The entry-point table needed a third errno to stop being partly vacuous.**
  ...
  those 68 cells; the other 28 compared two identical runs, so a widening of the
  arm — `EIO` joining it — left all 68 green. A non-permission control errno
  injected at the same stat fixes both halves: where the denial is delivered the
  control must *not* answer `PermissionDenied`, and where it is not, the injected
  errno must change nothing. The narrowing mutant still dies on 40; the widening
  mutant now dies on 32, measured against the same table one commit earlier,
  which passed all 68.
...
- [x] **BUG-265 — A refused SFTP connect raises `RemoteStoreError`, which contradicts fifteen docstrings and the health-check guide**
  ...
  **One backlog correction shipped with it:** this item's own body claimed
  ...
  still answers the wrong type. When this entry was written that was
  `PermissionDenied` on the `EACCES` shape and the base `RemoteStoreError` on
  the `EPERM` one; BUG-275 gave the mapping an `EPERM` arm, so both shapes now
  answer `PermissionDenied` — including the `EPERM` one, which is the shape a
  reader can actually produce. Either way it is not `BackendUnavailable`. The
  row is not unconditionally true and this entry does not claim it is.
...
- [x] **BUG-248 — BE-021's absent-container rule and GR-031's drive-identity escalation contradict each other**
  ...
  `write`, the one roster operation § Reach declines, plus `check_health`,
  ...
  **`GraphUtils.resolve_drive_id` was the same miss twice, and the second time
  ...
  enumerate: the space is (**leg** × error code) — not target shape × code, which
  is 4 × 2, since the site-URL and tuple shapes share a leg and each adds one —
  so ten cells, now a parametrised table rather than two sampled cells. Verified
...
- [x] **BUG-252 — Azure cassettes matched on a `blockid` the SDK mints, so `[azure]`'s drift smoke went red on a block-id change**
  ...
  **Shipped:** `blockid` added to the Azure profile's
  ...
  unrecorded block id, **one negative control** — an unrecorded query on a
  recorded path — and a third cell holding the `path` matcher.
  **Only the query cell is a control**, and the first shipped version of this
  guard did not have it: it offered the `path` cell as the control, and that
  is satisfied by the `path` matcher alone, holding even with `query` dropped
  from the match key entirely. Measured by running each candidate assertion
  under the shipped config and under
  ...
  generalises past this cassette, and it turns on a distinction the first
  version elided: deleting a **parameter** does not remove the **matcher** it
  sat under, it weakens it, so **the control has to vary that matcher through
...
- [x] **BK-338 — Decide what a PR review roster should be**
  spec: — · effort: S · audience: contributor.process
  ...
  **A sixth persona was considered and rejected.** The uncovered surface needs
  looking at, not owning; a persona whose domain is "everything left over" grows
  the roster on the axis ADR-0035 already found unpaying, and would have given
  those files an owner with no foundation docs for them.
  **Bound worth stating:** the finding-level classification is a per-finding
...
- [x] **BK-344 — Adapt `/ship` from PR #952's review evidence**
  ...
  **Co-shipped:** BK-338's "do not pin a model in a repo skill" is discharged
  (its roster question was left open here, annotated with signal 2's evidence,
  and closed by BK-338 itself immediately after — that annotation is what the
...
- [x] **BK-336 — `/fix-pr` should find a finding's siblings, not just the lines it names**
  ...
  **BK-338 stayed open here.** The reverted attempt bundled the review-roster
  question and a single-session model pin into the same commit; neither was
  touched by this item. Both have since shipped — the pin as part of BK-344, the
  roster as BK-338 itself, above.
...
- [x] **BK-319 — Cut PR CI wall-clock below 5 minutes without dropping guarantees**
  ...
  ~100 s in sequence) on all five interpreters in parallel, so the slowest
  interpreter set the wall-clock. Measurements that drove the redesign:
  oversubscribing xdist (`-n 8/12`) does **not** help — the suite is CPU-bound;
  sharding the *live-backend* suite is inefficient because each shard re-pays the
  per-test fixture setup (bucket create / container handshakes); the account is
  capped at **20 concurrent jobs** (no merge queue / no larger runners on a
  personal repo); and pytest-split's *call-time* durations mis-balance shards
  because live-backend and `azure_replay` VCR work happens in *setup* (a 2-way
  split ran one shard ~1.8× the other). Redesign (BK-319):
  the **primary** interpreter (3.13) runs the full live Stage-2 suite —
  pass-1 pytest-split-sharded ×2 with `least_duration` over a **setup-inclusive**
  `.test_durations_pass1` (generated by `scripts/gen_split_durations.py`) plus
  serial `sftp_docker` — and `coverage-gate` combines the shard partials to assert
  the **95 % floor unchanged**; the four **non-primary** interpreters run the
  repo-only **Stage 1** tier (no Docker) sharded ×2; the two ~45 s
  `TestCommittedCassettePIISweep` repo-hygiene scans (interpreter-independent) run
  once in `test-cassette-pii`. The full live-backend matrix on **every**
  interpreter is preserved off the per-PR path as the scheduled/on-master
  **`ci-full.yml`** backstop (documented in `sdd/CI-OPERATIONS.md`). Result:
  PR wall-clock **~4m15s** (measured 257 s, incl. formal/tla which a code PR
  skips), coverage 98 %. **Guarantee tradeoff (owner-approved):** a regression
  that manifests *only* on a non-primary interpreter *against a live backend* is
  caught by `ci-full` (within minutes of merge, or overnight) rather than blocking
  that individual PR; every other guarantee still gates each PR.
...
- [x] **BUG-232 — `[s3-pyarrow]` and `[bench]` float past the `pyarrow<25` ceiling**
  ...
  the `arrow` / `sql-query` / `s3-pyarrow` drift baselines refreshed to
  ...
  and 25, closing the standalone-extra gap. The review further flagged that the
  cap description was hand-copied into four docs (a principle-4 violation the lift
  itself exposed); the enumerated per-extra ceilings were removed from
  `FEATURES.md`, `CONTRIBUTING.md`, and the `drift_check.py` render-docs template
  so those now defer to `pyproject.toml` as the single source of truth, leaving
  the enumeration only in pyproject and immutable history. Surfaced in the PR #900
  and #903 reviews.
...
- [x] **ID-226 — Publish `llms-api.txt`: public-API skeleton for coding agents**
  ...
  async, and extensions included) with its exact signature, type annotations,
  and full docstring, bodies elided — served at `/llms-api.txt` next to the
  `mkdocs-llmstxt` `llms.txt` / `llms-full.txt`, for coding agents that *consume*
  remote-store. Generated by `lx -u -Y src/remote_store/` (public-only skeleton;
  `-e __pycache__` drops stray bytecode) at **Read the Docs build time**: `lx` is
  a Go binary, so RTD (pip-only) fetches it as a pinned **v1.2.2** prebuilt in a
  `build.jobs.post_build` step that writes `${READTHEDOCS_OUTPUT}/html/llms-api.txt`
  (non-fatal — a fetch/run hiccup never reds the docs build). Shipped **RAW** (no
  curation): the `-u` name heuristic also surfaces ~20 internal-but-public-named
  classes (transports, serializers, memory-tree nodes) alongside the real API —
  accepted as low-harm agent context; the per-backend guarantee value lives in
  the backend docstrings, so backends are kept rather than pruned to `__all__`.
  Linked from `llms.txt` (`markdown_description`), the README (`Learn more`), and
  the API-reference landing page. Sibling to **ID-216** (lx as *contributor*
  context, full source); this is the consumer-facing *public* cut. Deeper
  contract gaps (per-backend guarantees, concurrency, error retryability, cost
  model) are tracked separately in **ID-227**. **Post-deploy check:** confirm the
  live RTD build serves `/latest/llms-api.txt` — the binary fetch could not be
  verified locally.
