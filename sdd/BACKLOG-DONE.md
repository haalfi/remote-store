...
- [x] **BUG-188 — Benchmark SVG images broken on performance docs page**
  ...
  blob URL fallback and rendered as broken images. Fix: include common image
  globs (`*.svg`, `*.png`, `*.jpg`, `*.jpeg`, `*.gif`, `*.webp`) in the
  source map. Regression test: `test_build_source_map_includes_docs_src_images`.
