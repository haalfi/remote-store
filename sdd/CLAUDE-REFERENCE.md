...
|                            | **and `packaging/conda-forge/feedstock/recipe.yaml`**,    |
|                            | the copy the channel is served from — generated, so run   |
|                            | `hatch run gen-conda-feedstock` and commit it in the same |
|                            | change (`lint` and `docs-gate` fail on a stale one).      |
...
| Trigger                    | Also check / update                                       |
|----------------------------|-----------------------------------------------------------|
| **Bug fix**                | `sdd/BACKLOG.md` (item; the reproduction lives in its     |
|                            | dossier under `sdd/backlog/`, if it has one),             |
|                            | `CHANGELOG.md` (stub line under `[Unreleased]`), failing  |
|                            | test **before** the fix, spec if the bug contradicts a    |
|                            | spec invariant                                            |
...
|                            | (in `lint` and `docs-gate`): one entry per ID, each a     |
|                            | single `- <ID>:` line, and at most `_MAX_ENTRY_CHARS`     |
|                            | of prose (link targets not counted). It also gates the    |
|                            | other direction — every completed `user.`-tagged item     |
...
| Log a bug or improvement idea            | Add to the `sdd/BACKLOG.md` section whose promise it serves — it is not filed if none does, and there is no holding area. At most eight lines in the index; more goes in a dossier under `sdd/backlog/`. See [§ How this file works](BACKLOG.md#how-this-file-works) |
...
- **Backlog ID-set gate.** `python scripts/check_backlog_ids_vs_base.py --base origin/<BASE>`
  ...
  (a PR closing another branch's item). PR #997 did both, in rounds 5 and 7, and
  nothing caught either — `gen_backlogid.py --check` compares open IDs against
  *done* ones, so a vanished item passes it.
  **It belongs here rather than in a hatch bundle** because it reads
  `origin/<BASE>`, which only the branch-freshness check above guarantees is
  fetched and current; a `lint` entry would fail on a checkout without the ref,
  or — worse — compare against a stale one and under-report. Run it *after* that
  check, and note that `sdd/GATE-INVENTORY.md` therefore records it `advisory`:
  the inventory derives enforcement from `pyproject.toml` and the workflows, and
  a gate a skill runs is a home it cannot see. That bound is stated in the
  inventory itself.
