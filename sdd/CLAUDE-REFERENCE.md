...
| ADR                           | Regenerate `sdd/adrs/DIGEST.md` via `hatch run gen-adr-digest`; `preflight` runs `gen_adr_digest.py --check` |
...
| **ADR**                    | Run `hatch run gen-adr-digest`, commit                    |
| (add / edit `sdd/adrs/*.md`) | `sdd/adrs/DIGEST.md`. Freshness is gated by              |
|                            | `gen_adr_digest.py --check` in `preflight` (ID-233)       |
