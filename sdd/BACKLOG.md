...
- [~] **BK-002 — Glob / pattern matching strategy**
  Two-tier design chosen (ADR-0009): `Capability.GLOB` + `Backend.glob()` for
  backends with native support (Local via pathlib); `ext.glob.glob_files()` for
  universal client-side fallback. S3/Azure prefix-optimized implementations
  remain as future work.
  Related: ID-007.
  → Spec: `sdd/specs/018-glob.md` (extends `003-backend-adapter-contract.md`)
  → ADR: `sdd/adrs/0009-glob-two-tier-design.md`
...
- [~] **ID-007 — `Store.glob()` surface API**
  `Store.glob(pattern)` implemented (capability-gated on `Capability.GLOB`).
  `ext.glob.glob_files()` provides universal fallback. Local has native glob;
