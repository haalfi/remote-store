...
**true** (BK-339, BK-325, BK-364, ID-125, BK-374), **reachable** (BK-327), and
...
- [ ] **BK-374 — The Tested-versions page answers a question next to the one a user brought, and is silent about the extras it skips**
  ...
  **That is not the contradiction it looks like.** The dependency-policy page's
  ...
  breach of it.
  **Second gap, same page: the exclusions are invisible.** The page renders a
  section per covered extra and says nothing about the rest, so `[toml]` — a
  user-facing extra in the README's install list — simply has no row, which
  reads as "nothing changed" rather than "never checked". The policy page
  states the exclusion as a property (an extra whose resolution depends on the
  running interpreter) without naming which, because naming them in prose would
  be the enumeration [`CONTENT-RULES.md` Rule 2](CONTENT-RULES.md#rules)
  forbids. Generated, it is not an enumeration in prose.
  **Both are derivable from what the generator already loads**, which is why
  this is S and not M: `list_extras()` returns the covered set,
  `_EXCLUDED_EXTRAS` the remainder, and `drift_smoke_map.SMOKE_TARGETS` plus
  its `--import-only` fallback give the depth each covered extra is exercised
  at. `_direct_deps_for()` walks the extras table already but returns names
  only, discarding specifiers, so emitting ranges needs a sibling that keeps
  them — a small addition, not a new data source.
