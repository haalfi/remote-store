...
v0.28.0 added the Microsoft Graph backend (ID-127): OneDrive, SharePoint, and Teams document libraries behind the same `Store` API as a local folder. It is the first backend with no synchronous face at all — you construct it with `AsyncStore(backend=GraphBackend(...))`, and there is no config `type=` string, because Graph's SDK story is `httpx` + `msal` and forcing a sync wrapper around an async-native transport would have been a lie told for symmetry. The work landed as a chain of milestone slices — foundation, core surface, read, transfer-resilience, write, mutate, docs/e2e — each its own PR with conformance tests, the way the v0.24.0 retrospective's "contract-expanding feature" DoD prescribes. That part went to plan. The part that did not was testing.

**There is no Azurite for Graph.** Every prior backend had a free local emulator or a moto-style in-process fake: S3 has moto, Azure has Azurite, SFTP has a Docker openssh image. Graph has nothing — the only thing that speaks the Graph API faithfully is Graph, and Graph requires a real tenant, real OAuth, and real network. So the testing strategy had to be built before the backend could be trusted. The answer was cassette replay (BK-262): record real HTTP traffic once against a live tenant, scrub it, commit it, and replay it deterministically in CI with `respx`/`vcrpy`. That sounds simple and was not. BK-284 redesigned the recording pipeline around declarative cassette profiles after the first ad-hoc version proved unmaintainable, and migrated the header scrubbing onto vcrpy's `filter_headers` — because the first cut keyed its redactions on the JSON body and sailed right past the same secrets sitting in `Authorization` and `Location` headers. BK-283 went further and drove the *published example snippet* from replayed cassettes in CI, so the code in the Graph guide is executed against recorded traffic on every run rather than being prose that rots. Cassettes gave cheap, deterministic, offline coverage of the request/response shapes. What they could not give was concurrency.

**The concurrency bugs only existed when two writers were real.** A mock returns what you told it to return; it cannot race. The create-race contract for `overwrite=True` — last-writer-wins when two clients create the same key at once — was specified, mocked, and green long before it actually worked, because the failure mode is a `409` (or, for large files, a mid-session `404`) that only one of two genuinely-concurrent writers ever sees. BK-294 added the small-file retry; BK-296 extended it to the large-file upload session and was where the live findings got expensive. With the retry wired but reusing the small-file backoff constants, a four-way 5 MiB same-key race converged **zero** times out of five: a whole-file re-upload takes seconds, so a sub-second backoff cap just dumps every retrier back into the middle of someone else's upload, and the aggressive cadence tripped Graph's own throttling on top. Lengthening the backoff while keeping the attempt count made it 2/5. Only a path-specific budget — the large path gets eight attempts at a 0.5s base and a 6.0s cap, the small path keeps its fast sub-second cap — got it converging in a live sample, and even then it is documented as **best-effort**: each re-attempt re-uploads the entire body, so the budget is bounded and a loser can still exhaust to `AlreadyExists`. None of that calculus is derivable from a cassette. It came from running `RS_TEST_LIVE_GRAPH=1` against a real drive and watching the success count.

**Credential hygiene had a new shape here too.** Graph's large-file upload hands you a pre-signed `uploadUrl` that *is* a bearer credential for the session, and the first version leaked it into a `ResourceLocked` error message when a mid-session upload was rejected (BK-263). The `_SENSITIVE_KEYS` masking set was widened for `client_secret` / `client_certificate`, and the cassette scrubbing had to learn the same lesson the masking layer already knew: a secret is a secret wherever it appears, and a redaction rule that only looks in one place is a redaction rule that misses. Three audits rode along to keep the new surface honest — a Graph backend audit, a CI-operations audit, and a dev-process gate-topology audit — each filing its findings as backlog items rather than silent fixes, per the project's report-only audit rule.

**The cross-backend payoff.** Adding a backend whose concurrency story was this fraught forced the question every prior backend had answered only implicitly: what does each backend actually promise under concurrent use? BK-287 turned that into a contract — `GR-059` plus per-backend `thread_safe` / `single_connection` clauses — and BK-289 built a posture-gated conformance lane so the promise is tested, not just documented (BK-288 wrote the user-facing version). A bug found in the new async backend thus paid out as a clarification that covers all of them. That is the recurring shape of this project: the hard backend is the one that makes the shared contract precise.
