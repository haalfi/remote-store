...
**The lesson, captured as `feedback_dafny_spec_test_interlock.md`:** the value of a formal layer is not "second backend to diff against" but "machine-checked interlock between spec and test." The compiled oracle (run as a conformance fixture) certifies that tests demand nothing the contract does not — it is the (T) side. The contract postconditions, verified against the `MemoryBackend` refinement, certify that a spec clause is self-consistent and satisfiable — it is the (C) side. ID-206 made the (C)/(T) coverage measurable; BK-196/BK-232/BK-195/BK-233 demonstrated the pattern on a real defect.

### Phase 40: Finishing the File-Ancestor Contract (v0.27.0)

v0.27.0 was the release where a single formal invariant propagated outward into every backend's error-translation path. v0.26.0's ID-190 had confronted one empty-key round-trip divergence the formal contract refused to wave through; v0.27.0 closed it (BK-234) and turned "write under a path that runs through an existing file" from a backend-dependent grab-bag of native exceptions into one cross-backend contract — proven, backend by backend, on each backend's own terms.

**A class invariant made a defensive postcondition load-bearing.** ID-184 had added an `AllAncestorsTraversable` disjunct to the Dafny listing postconditions, but the state it guarded was structurally unreachable from the public `Backend` trait, so every backend satisfied it vacuously — a verified guard proving nothing. ID-209 promoted well-formedness to a `Valid()` class invariant on the trait: every key's slash-aligned ancestors are directories, `requires Valid() ensures Valid()` on every mutating method. That made the disjunct a logical consequence rather than a defensive afterthought, and enforcing it surfaced a real cross-backend bug the vacuous version had hidden. Local and SFTP leaked native `FileExistsError` / `NotADirectoryError` / SFTP `ENOTDIR` when a write descended through a file — a BE-021 violation. Both now map to `InvalidPath` (read/delete map to `NotFound`), and the conformance gate is certified through the compiled Dafny oracle.

**The contract had to be re-proven on every backend's own native-error terms.** ID-209 deliberately carved out the backends whose native errors did not already match, and closing each was its own investigation — not a copy of the last fix. Azure HNS (ID-213) raised `ResourceNotFoundError` / `ResourceExistsError`, which the classifier mapped to `NotFound` / `AlreadyExists` — the wrong class — and worse, the move/copy error was keyed to the *source* path during the DFS rename, carrying no signal about the destination file-ancestor the contract actually names. The classifier was the wrong seam; the fix is a per-method ancestor probe that inspects `hdi_isfolder` so a directory marker is not mistaken for a file. SFTP (ID-212) walked the parent chain from the absolute SFTP root, so on a chrooted server a permission-denied ancestor *above* the chroot masked a genuine file-ancestor case; the walk now starts at `base_path` and never probes what it cannot see. The flat-namespace backends (ID-211) cannot detect a file ancestor at all without an extra HEAD per slash-aligned ancestor, so the gate ships as an opt-in `reject_write_under_file_ancestor` flag with measured cost numbers (~+9–19 ms at depth 6), off by default, and short-circuited entirely for paths with no slash. One contract, four native realities.

**An audit found an atomicity hole the unit suite could not see.** The ID-200 s3fs error-mapping audit turned up BUG-214: when the content source failed mid-stream, both S3 backends committed a complete-looking but *truncated* object, because s3fs's `S3File.close()` and PyArrow's output stream both finalize the upload on close regardless of the in-flight exception — a direct `ATOMIC_WRITE` violation. The fix discards the in-flight multipart upload (s3fs) or buffers the content fully before opening the upload (PyArrow), and was confirmed against real AWS for *both* backends via a new `s3_pyarrow_live` Stage-3 fixture. The same audit's listing-cache spike (ID-201) found that s3fs's never-expiring `DirCache` made cross-writer staleness permanent and total (25/25 on moto and MinIO), so v0.27.0 flips the s3fs lanes to fresh listings by default (BK-257) and leaves the cache as an explicit opt-in.

**Internal tracker IDs stopped leaking into the public API reference.** A user spotted a docstring in the rendered reference that read "See spec 003 § BE-008 and ID-211" — internal coordinates that mean nothing to a reader and date the prose the moment a ticket closes. The audit found 178 such leaks across 24 files; BK-246 rewrote each as behaviour-first prose and added a `check_no_tracker_refs` lint gate, dual-wired into `hatch run lint` and CI, so the leak cannot recur. BK-250 closed the matching gap on the spec side: five shipped-but-untested behaviours got conformance tests, and the HTTP `LAZY_READ` divergence resolved *toward the code* — the flag was always truthful because `read()` returns a live streamed body on every transport; the spec clause and docstring had simply under-listed it.

**The lesson, captured as `feedback_cross_backend_contract_per_backend_proof.md`:** a cross-backend behavioural contract is finished only when it has been re-proven on each backend's own native-error surface, never by assuming one backend's fix generalizes. The four file-ancestor follow-ups looked like one task split four ways; they were four distinct investigations — a classifier that was the wrong seam, a walk that started in the wrong place, a cost trade-off with no native detection, and a native-exception leak — sharing only the contract they converged on. The emulator-versus-real-cloud lesson from v0.25.0 recurred here too: Azure HNS's wrong error class only surfaced against a live account, because the seam the contract needed (source-keyed move errors) was invisible to the recorded fixtures until a real run produced it.
