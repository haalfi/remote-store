...
### Phase 45: What a Failure Is Allowed to Mean (v0.31.0)

v0.31.0 is the release where the project stopped letting a backend answer a failure with whatever its wire protocol happened to reveal. Its through-line is one question asked of many operations: when the thing you are talking to is silent, absent, or refusing, what exactly is the caller told, and did anyone measure it rather than read it?

**It started with a user whose transfer took seventy minutes.** Issue #970 reported an SFTP pull that could not tell "slow" from "stalled": a 214 MB file took twenty minutes and a 2 GiB file seventy, so elapsed time was useless as a signal, and a peer that finished the SSH handshake and then went quiet blocked forever with no exception and no log line. The fix was a byte-level bound, `io_timeout` (BK-354), on silence *between* bytes rather than on the transfer, so a slow link never trips it. Shipping it opt-in was the compatible choice and the wrong one: the reporter said so, the library's own `ReadOnlyHttpBackend` already defaulted a bound, and the SFTP recovery path had been written assuming one existed. Flipping the default to 120 s (BK-356) is this release's headline breaking change, and it could not ship until two things it would have exposed were fixed first. Closing a failed stream re-entered the dead channel and paid the bound twice (BK-355), and a seek-to-end on a stalled channel silently answered `0` for a file of any size, because paramiko discards its own stat failure (BK-357). A real bound would have turned that hang into a wrong answer, which is worse than a hang.

**Then the review loop kept running the failures instead of reading them, and each run found the next one.** A stall raised `BackendUnavailable` with an empty message and no record, because `socket.timeout()` carries no text (BK-359); the same blank shape existed on Azure (BUG-264). A refused port and a DNS failure raised the base error that fifteen docstrings promised they would not (BUG-265). A dropped connection mid-read leaked paramiko's own exception from a `read()` stream while `read_bytes()` mapped the same drop correctly (BK-358). The rename fallback for servers without `posix-rename` cleared the destination under a suppression, so a promote that failed for any non-dead reason destroyed the old file and then cleaned up the temp, leaving no copy of either (BUG-272, BUG-270, BUG-277). A denial spelled `EPERM` raised a different type from one spelled `EACCES` (BUG-275). And an operation against a host that was never reached paid the whole connect retry budget two or three times over, because the re-entry guards asked only whether an established connection had dropped (BUG-274, BUG-278). None of these came from a bug report. Each was found by a review round that staged the failure against a relay or a fake transport and measured what the caller got, and the entry for BK-356 records seven rounds and forty-six findings with no defect in the one-line implementation, because every finding was in what the prose claimed about it.

**The other half of the release asked the same question about absence.** `delete(missing_ok=True)` returned silently against a missing bucket while `delete_folder(missing_ok=True)` raised on the same store, because no clause had ever spoken of the container holding the path (BUG-243). Deciding that an absent container reads as an absent path rippled through four backends whose probes raised where the contract said `False` (BUG-246), a `LocalBackend` that reported its own deleted root as a path escape (BUG-247), a Graph clause that deliberately said the opposite and had to be adjudicated in an ADR rather than fixed (BUG-248, filed at two operations and measured at eleven), and an SFTP store that, asked to write to a root that did not exist yet, wrote the bytes *at* the container and left it a regular file (BUG-259). The flat-namespace reconciliation (BK-324) sits underneath all of it: the S3 family, flat Azure and the SQL backends had been answering a file operation aimed at a folder with `NotFound`, silence, or success, and now raise `InvalidPath` like everyone else. Every one of these entries has a section in the migration guide, and this is the release where that stopped being a checklist step nobody read: the PR making a break now owes its section and a gate refuses a breaking entry that does not link one (BUG-261, BUG-262).

**The lesson:** "the backend raises the right error" is not a property you can read off the code, because the code only shows what it does with the exceptions it thought of. It is a property of the space of failures, and the only way this release found any of its bugs was to enumerate that space and drive it: transport dead at each round trip, denial at each stat site, container absent under each operation. The measuring reviewer was the one who kept being right, and the entries that record "four rounds of prose agreed on the two-operation framing; one command refuted it" are the release's real deliverable. A failure mode you have not staged is one you have not specified.
