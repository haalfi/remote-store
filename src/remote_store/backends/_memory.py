...
class MemoryBackend(Backend):
    ...
    def read(self, path: str) -> BinaryIO:
        ...
        The value materialises in memory rather than streaming (``LAZY_READ`` is
        not advertised): the returned stream wraps a copy of the resident bytes,
        so peak memory scales with the object size. Access is guarded by a single
        process-wide lock.
    ...
    def read_bytes(self, path: str) -> bytes:
        ...
        Copies the resident value out under the lock; like ``read`` it holds the
    ...
    def write(
        ...
        *,
        ...
        The whole body is buffered into memory before the store (streams are
        drained fully first — no lazy write), and the swap-in happens under a
        single process-wide lock, so the write is atomic with respect to
    ...
    def write_atomic(
        ...
        *,
        ...
        Memory writes are already atomic under the process-wide lock, so this is
        exactly ``write``; the whole body is buffered first.
    ...
    def open_atomic(self, path: str, *, overwrite: bool = False) -> Iterator[BinaryIO]:
        ...
        ``write`` under the process-wide lock, so *path* updates in one atomic
        step. An exception before exit leaves *path* untouched.

        Raises:
            AlreadyExists: If a file exists at *path* and ``overwrite`` is ``False``.
            InvalidPath: If *path* is empty, or (on commit) names a folder or has
                a file ancestor.
        """
        segments = self._split_path(path)
        if not segments:
    ...
    def delete(self, path: str, *, missing_ok: bool = False) -> None:
        ...
        Raises:
            ...
            InvalidPath: If *path* is empty or names a folder (a type mismatch
                ``missing_ok`` does not silence).
        """
        segments = self._split_path(path)
        if not segments:
            raise InvalidPath("Path must not be empty for file operations", path=path, backend="memory")
    ...
    def delete_folder(self, path: str, *, recursive: bool = False, missing_ok: bool = False) -> None:
        ...
        ``recursive=True`` detaches the whole subtree in one locked step;
        ``recursive=False`` removes only an empty folder.
    ...
    def list_files(
        ...
        *,
        ...
        A missing or non-folder *path* yields nothing. The tree is snapshotted
        under the lock and iterated outside it, so a long listing does not hold
        the lock; ``recursive`` walks the whole subtree (``max_depth`` prunes it).
        """
        segments = self._split_path(path)
        with self._lock:
            node = self._traverse(segments)
    ...
    def list_folders(self, path: str) -> Iterator[FolderEntry]:
        ...
        A missing or non-folder *path* yields nothing; children are snapshotted
        under the lock and yielded outside it.
        """
        segments = self._split_path(path)
        with self._lock:
            node = self._traverse(segments)
    ...
    def iter_children(self, path: str) -> Iterator[FileInfo | FolderEntry]:
        ...
        Overrides the base two-pass default: takes one locked snapshot of the
        directory and yields ``FileInfo`` for files and ``FolderEntry`` for
        folders. A missing or non-folder *path* yields nothing.
        """
        segments = self._split_path(path)
        with self._lock:
            node = self._traverse(segments)
    ...
    def get_folder_info(self, path: str) -> FolderInfo:
        ...
        walking the whole subtree, so cost scales with the number of descendants.
    ...
    def move(self, src: str, dst: str, *, overwrite: bool = False) -> None:
        ...
        Detach-from-source and attach-to-destination happen in one locked step,
        so the move is atomic with respect to concurrent callers (``ATOMIC_MOVE``
        is advertised) and the payload is never copied. ``src == dst`` verifies
        the source is a file and is otherwise a no-op.
    ...
    def copy(self, src: str, dst: str, *, overwrite: bool = False) -> None:
        ...
        The destination receives a fresh ``bytearray`` copy of the source bytes,
        all in one locked step (atomic). ``src == dst`` is a no-op.
