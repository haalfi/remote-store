...
# Per RFC 4253 § 7.1, SSH_MSG_KEXINIT carries ten name-lists in this
# fixed order after the 16-byte cookie.
_KEXINIT_FIELDS: tuple[str, ...] = (
    "kex_algorithms",
...
def _recv_exact(sock: Any, n: int) -> bytes:
    """Read exactly *n* bytes from *sock* or raise ``OSError`` on short read."""
    buf = bytearray()
    while len(buf) < n:
        chunk = sock.recv(n - len(buf))
        if not chunk:
            raise OSError(f"unexpected EOF: needed {n} bytes, got {len(buf)}")
        buf.extend(chunk)
    return bytes(buf)
...
class SFTPUtils:
    ...
    - ``SFTPUtils.scan_host_algorithms(host, port=22)`` -- raw-socket SSH
      KEXINIT probe; returns the server's algorithm advertisement
      (kex / host-key / cipher / MAC / compression name-lists) for
      diagnosing ``IncompatiblePeer`` failures
...
class SFTPBackend(Backend):
    ...
    def check_health(self) -> None:
        ...
        Establishes the SSH/SFTP connection lazily if needed (retried at
        connection scope) and issues one ``stat`` round-trip.
    ...
    def write_atomic(
        ...
        *,
        ...
        Readers never observe a partial file: the body is streamed to a hidden
        ...
        the target is moved aside first, and moved back if the rename fails).
        ...
        Ordinarily the restore succeeds and the old content is back at its own
        path. It is best-effort, though: a dropped connection stops it being
        ...
        with self._errors(path):
            ...
            try:
                ...
                # (audit-020 L4, mirroring ``open_atomic``'s guard). If the failure
                # is a dropped-connection signal, ``remove`` would re-enter the
                # ``_sftp`` property and, against a down server, run the full
                # tenacity reconnect/backoff inside this suppressed cleanup only for
                # ``_errors``/``_map_exception`` to null the client immediately
    ...
    def open_atomic(self, path: str, *, overwrite: bool = False) -> Iterator[BinaryIO]:
        ...
        *path* holds the new content; and on the rename-fallback path a dropped
        ...
        try:
            ...
            # gone. Only an ``Exception`` can be a connection death — a
            # ``GeneratorExit`` / ``KeyboardInterrupt`` (BaseException, not
            # Exception) is an interrupt, never a backend error.
            connection_lost = isinstance(exc, Exception) and (
                self._is_connection_dead(exc) or isinstance(exc, paramiko.SSHException)
            )
            # Best-effort temp cleanup — but never at the cost of a reconnect.
            ...
            # dead channel regardless. This cleanup now also runs for a
            # GeneratorExit / KeyboardInterrupt (audit-020 L5): an abandoned or
            # GC'd ``with open_atomic(...)`` block would otherwise leave the temp
            # file littering the server. The atomic contract already held
            # (``_promote`` never ran → target untouched); this only removes litter.
            if self._sftp_client is not None and not connection_lost:
                with contextlib.suppress(Exception):
                    self._sftp.remove(tmp_path)
            ...
            # connection signals) and any interrupt (GeneratorExit /
            # KeyboardInterrupt) propagate unchanged.
            # ``connection_lost`` already implies ``isinstance(exc, Exception)``;
            # the redundant check narrows the type for ``_map_exception`` (which
            # takes ``Exception``, never a bare ``BaseException`` interrupt).
            if isinstance(exc, Exception) and connection_lost:
    ...
    def delete_folder(self, path: str, *, recursive: bool = False, missing_ok: bool = False) -> None:
        ...
        ``recursive=True`` walks and removes the subtree bottom-up (one round-trip
        per entry — not atomic; an interruption can leave the tree partially
        removed). ``recursive=False`` removes only an empty folder after checking
        it has no entries.
    ...
    def list_files(
        ...
        *,
        ...
        Lazily walks the remote directory (``listdir_attr``); a missing *path*
        yields nothing. ``recursive`` descends via one directory-listing
    ...
    def list_folders(self, path: str) -> Iterator[FolderEntry]:
        ...
        One directory-listing round-trip; a missing *path* yields nothing. Other
        failures surface during iteration, mapped as in ``list_files`` — a denied
        listing raises ``PermissionDenied`` (``EACCES`` or ``EPERM``).
        """
        try:
            sftp_path = self._sftp_path(path)
            try:
    ...
    def iter_children(self, path: str) -> Iterator[FileInfo | FolderEntry]:
        ...
        Overrides the base two-pass default with a single ``listdir_attr``
        round-trip, yielding ``FileInfo`` for files and ``FolderEntry`` for
        folders. A missing *path* yields nothing; other failures surface during
        iteration, mapped as in ``list_files`` — a denied listing raises
        ``PermissionDenied`` (``EACCES`` or ``EPERM``).
        """
        try:
            sftp_path = self._sftp_path(path)
            try:
    ...
    def get_folder_info(self, path: str) -> FolderInfo:
        ...
        recursively walking the whole subtree (one listing round-trip per
        folder), so cost scales with the number of descendants.
    ...
    def move(self, src: str, dst: str, *, overwrite: bool = False) -> None:
        ...
        Tries ``posix_rename`` first (atomic on POSIX-compliant servers), then a
        plain ``rename``, and finally a stream copy-then-delete. Because the
        outcome depends on server support, atomicity is not guaranteed across all
        ...
        with self._errors(src):
            ...
            try:
                ...
                if self._is_connection_dead(exc):
                    ...
                    # One bound, measured — not the four this comment used to
                    # enumerate: the displace re-raises rather than suppressing,
                    # and the copy rung was already unreachable behind
    ...
    def _raise_if_dir(self, sftp_path: str, path: str, *, cause: Exception | None = None) -> None:
        ...
        a **permission** error (``EACCES`` / ``EPERM``) is re-raised so a server
        ...
        *cause* is the failure the caller is classifying, where it has one. When
        ...
        try:
            ...
            # BK-316 (L1): the classification stat's *own* failure normally must
            # not override the caller's original error — a raced-away target
            # (``ENOENT``) or an errno-less ``SSH_FX_FAILURE`` (the ambiguous shape
            # the caller's file-ancestor recheck handles downstream, L3/L6) is
            ...
            # failure with a different errno. Both errnos are re-raised for
            # ``_map_exception`` to classify, which answers each with
            # ``PermissionDenied``.
    ...
    def _classify_existing_target(self, st: Any, path: str) -> None:
        ...
        represent an unsupported/absent mode field) is treated **defensively** as
        ...
        *every* rename, so on that server class this stat is paid once per
        ...
        try:
            ...
            if self._is_connection_dead(exc):
                ...
                # handed is already a drop (mechanism 1, and it predates this
                # displace), and the fallback's ``rename`` is never reached
                # because the displace re-raises first — so the displace is the
    ...
    def _displace(self, sftp_path: str) -> str | None:
        ...
        **Cost.** None on the ordinary path. On a failed displace the ``and``
        ...
        ``SSHException`` shape of a drop needs no arm — it is not an ``OSError``,
        so it never reaches the ``except`` at all.
        """
        name = sftp_path.rsplit("/", 1)[-1] if "/" in sftp_path else sftp_path
        parent = sftp_path.rsplit("/", 1)[0] if "/" in sftp_path else "."
    ...
    def _restore(self, backup: str, sftp_path: str, exc: BaseException) -> None:
        ...
        Skipped when *exc* says the channel is dead, on the same reasoning — and
        with the same predicate, which is the part that has to be copied rather
        than approximated — as the temp cleanup in ``write_atomic`` /
        ``open_atomic``: the rename cannot arrive, and reaching for ``_sftp``
        would run the full reconnect inside the error path. ``SSHException`` is
        in that predicate because ``_is_connection_dead`` deliberately leaves it
        ...
        **The target is cleared first, and that is not the window reopening.**
        ...
        displace succeeded, so the caller's file is in *backup*. It is reachable
        — ``_move_fallback``'s copy rung opens the destination ``"w"`` before it
        can fail — and the servers this fallback exists for are the ones that
        refuse a rename onto an occupied path, so without the remove the restore
        would fail on exactly the class of server that needs it.
        """
        import paramiko
        ...
        if self._sftp_client is None or connection_lost:
            return
        with contextlib.suppress(Exception):
            self._sftp.remove(sftp_path)
        with contextlib.suppress(Exception):
    ...
    def _release(self, backup: str) -> None:
        ...
        failure here leaves litter, not a wrong result. ``Exception``, not
        ``OSError`` — a drop between the promote and this unlink arrives as
        ``SSHException`` or ``SFTPError``, neither of which is an ``OSError``,
        and letting one out would report a completed write as failed.

        There is no *exc* to test here, so the dead-channel skip the other
        best-effort sites make is the transport's own flag instead: ``_sftp``
        would otherwise run the tenacity reconnect after the caller's operation
        has already succeeded. ``_is_connected`` is a local flag read, so asking
        costs nothing.

        **It is a weaker guard than the ones keyed on an exception, and the gap
        is measured.** ``_is_connected`` reads the *transport*, so a channel that
        goes silent under a live transport reads as active — the case
        ``io_timeout`` exists for. The unlink is then attempted and blocks for
        one bound before being suppressed: measured 2.00 s at a 2.0 s bound, on a
        call that had already succeeded and still returns its result. The other
        sites do not have this shape because they hold a failure to test; here
        there is none, and probing the channel to find out would cost the same
        round-trip the guard is trying to save. So this is the accepted cost of
        tidying up, recorded rather than hidden: the backend's one-bound
        guarantee is about the *failure* path and does not cover it.
        """
        if self._sftp_client is None or not self._is_connected():
            return
        with contextlib.suppress(Exception):
    ...
    def _move_fallback(self, src_sftp: str, dst_sftp: str, *, overwrite: bool) -> None:
        ...
        destination while it stands displaced, so releasing the backup before the
        copy has finished would put the fallback back in the window it exists to
        close.

        **The two are not symmetric on failure, and this is the asymmetric one.**
        A failed promote in ``_rename_fallback`` leaves the destination free,
        because the rename that would have occupied it is the thing that failed.
        Here the copy rung opens the destination ``"w"`` before it can fail, so a
        failure after that point leaves the path occupied by a truncated copy —
        which is why ``_restore`` clears the target rather than renaming onto it.
        """
    ...
    def _base_relative_ancestor_dirs(self, sftp_path: str) -> Iterator[str]:
        ...
        (inclusive) down to its immediate parent.

        The walk root is ``self._base_path`` — the configured, connection-time
        root — never the absolute SFTP root ``/``.  Components at or above the
        base are never yielded, so a chrooted SFTP deployment that returns
        ``SSH_FX_PERMISSION_DENIED`` for an ancestor above the chroot does not
        derail the walk.  ``_has_file_ancestor`` and ``_ensure_parent_dirs``
        share this iteration so their walks stay paired.

        When ``base_path`` is ``"/"`` there is no chroot boundary above the
        SFTP root, so every component of *sftp_path*'s parent chain is yielded.
        """
        parent = sftp_path.rsplit("/", 1)[0] if "/" in sftp_path else ""
        if not parent:
            return
        ...
        if base == "/":
            ...
            return
        if parent == base:
            # Immediate parent is the base itself: no intermediate ancestors,
            # and the base is assumed to exist (the connection-time root).
            return
        if not parent.startswith(base + "/"):
            return  # defensive: path is not under base — nothing to walk
        yield base
        current = base
        for part in parent[len(base) + 1 :].split("/"):
            if part:
                current = f"{current}/{part}"
                yield current

    def _has_file_ancestor(self, sftp_path: str) -> bool:
        """Return True iff any ancestor of *sftp_path* below ``self._base_path``
        ...
        The walk starts at ``self._base_path`` (see
        ``_base_relative_ancestor_dirs``) so it never stats an ancestor
        above the configured root — including the chroot boundary on a
        partial-permission SFTP deployment.  Only fires on the slow error
        path, so the per-stat-per-ancestor round-trip cost is paid once
        per failing read, not per successful read.
        """
        for current in self._base_relative_ancestor_dirs(sftp_path):
            try:
                ...
                # distinguish e.g. EACCES and kept it deliberately: being able to
                # stat a directory but not its regular-file child is unusual, and a
                # false "no file ancestor" here only forgoes the NotFound upgrade —
                # the caller's original error still surfaces mapped (ID-209).
                return False
    ...
    def _ensure_parent_dirs(self, sftp_path: str) -> None:
        ...
        Walks ``self._base_path`` (inclusive) down to the immediate parent via
        ``_base_relative_ancestor_dirs``; ancestors above the base are never
        probed, so a chrooted server that denies stat above the chroot does
        not derail directory creation.
    ...
    def _is_unreachable(exc: Exception) -> bool:
        ...
        *working* channel on any denied operation: paramiko's
        ...
        errno dispatch, which this module also calls the errno dispatch and
        which does answer ``EPERM``; see below.)
        It stays excluded anyway, because the errno dispatch below now answers
        both with ``PermissionDenied``, and claiming either here would take that
        away and clear the client with it. That the widening was also tried and
        measured harmful — after a revision of this docstring claimed ``EPERM``
        was free to take, true of the errno dispatch as it then stood and false
        of the module — is the second reason, not the only one.

        **What both errnos still get wrong is connect-time**, and it is the same
        wrong for each: a connect this machine rejected locally is answered as a
        denial naming the caller's key. Fixing it needs context only ``_connect``
        has, so it is tracked as its own item; the two being alike is what lets
        one fix reach both.
        """
        import paramiko
    ...
    def _map_exception(self, exc: Exception, path: str) -> RemoteStoreError:
        ...
        if isinstance(exc, OSError):
            ...
            if code in (errno.EACCES, errno.EPERM):
                # BUG-275: both permission errnos, because the alternative does
                # not converge. Every operation in this module classifies through
                # here, so this is the one place the answer does not depend on
                # which of the module's stats or opens the server refused.
                # Guarding those sites individually was tried twice and each
                # subset drew a boundary a caller cannot see — first between
                # ``write``'s two overwrite modes, then between ``read_bytes``
                # and ``get_file_info`` on one path and one denial. The arm also
                # puts SFTP where ``LocalBackend`` already is: it catches bare
                # ``PermissionError``, which CPython raises for both errnos.
                #
                # The cost is stated rather than hidden: this arm sees only the
                # exception, so a *connect-time* ``EPERM`` — the shape a
                # netfilter ``REJECT`` on the ``OUTPUT`` chain reproduces — is
                # answered as a denial naming a key that had no part in it. That
                # is already true of ``EACCES`` here, and making the two alike is
                # what lets one fix reach both: the repair needs connect-time
                # context only ``_connect`` has, and is tracked as its own item.
        ...
        if isinstance(exc, paramiko.ssh_exception.IncompatiblePeer):
            ...
            # they can identify which name-list the server narrowed before
            # reaching for connect_kwargs["disabled_algorithms"]. The literal
