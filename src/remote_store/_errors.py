...
class PermissionDenied(RemoteStoreError):
    ...
    **Not always the storage system**: on SFTP the error mapping sees only
    the exception, so a connect the local machine refuses — a firewall rule
    — is reported as a denial too, naming the key that was asked for. Read
    this type as "denied", not "denied by the server"; distinguishing the
    two needs connect-time context and is tracked separately.
    """
