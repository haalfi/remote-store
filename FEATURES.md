...
| `memory` | In-memory store for testing and caching | `MemoryBackend` | — | Yes | All except GLOB and LAZY_READ |
| `http` | Read-only HTTP file access (stdlib urllib; optional `requests`/`httpx` adapters) | `ReadOnlyHttpBackend` | — (`requests`, `httpx` optional) | Yes | READ, METADATA, LAZY_READ |
