...
- BUG-274: **Fix** — an SFTP operation against a host that cannot be reached pays the connect retry budget once instead of two or three times. At shipped defaults a `read_bytes` against a refused port drops from 8.00 s to 4.00 s, and a nested key from 12.01 s to 4.00 s
