...
- BUG-254: **Fix** — `DagsterComputeLogManager.get_log_keys_for_log_key_prefix` is annotated `Sequence[list[str]]` rather than `Sequence[Sequence[str]]`, the narrowest annotation valid under every supported dagster. No behaviour change: the method already returned `list[list[str]]`. Callers annotating the result as `Sequence[Sequence[str]]` are unaffected; the narrower return satisfies the wider annotation
