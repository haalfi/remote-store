...
- ID-230: Present benchmark overhead, don't judge it. `bench-report --user` now reports a neutral magnitude band (`sub-ms`/`<10%`/`10-50%`/`>50%`) plus a factual faster/slower direction instead of an acceptability verdict (`Negligible`/`Favorable`/…); the performance guide and README lead with the measured delta, leaving the acceptability call to the reader (the overhead *mechanism* is corrected in BK-314). The published `comparative.md` and the four charts are refreshed from a reproducible Linux/Docker **run of record** (committed source JSON under `benchmarks/results/run-of-record/`, replacing the stale 2026-04-12 Windows-laptop numbers whose chart source was never committed); a `workflow_dispatch` `run_of_record` job + `hatch run bench-run-of-record` slim/guard rebuild them from committed inputs
- BK-314: Present benchmark overhead in milliseconds, not as a percentage, and fix the "shrinks as a share" narrative. Both overhead charts (`overhead.svg`, `overhead-vs-rtt.svg`) now plot absolute ms overhead (remote-store minus raw SDK) instead of a percentage of raw time, and a new decomposition chart (`overhead-decomposition.svg`) stacks raw SDK time + remote-store overhead per RTT profile, labelling the overhead in ms and as a share of total. The performance guide and README replace the false "a fixed per-op cost shrinks as a share / 1 ms on a 100 ms round trip is 1%" premise with the accurate mechanism: remote-store's overhead is a count of extra protocol round trips (BK-313), so its ms cost *scales* with round-trip time — an S3 write or delete carries about one extra round trip of overhead (~+90 to +110 ms at 100 ms RTT), while backends that add no extra round trips per operation stay near zero at any latency. Its share of total time can rise or fall, but the absolute cost grows. Still presents the overhead without judging acceptability (ID-230 framing)
