...
- BUG-243: **Change** — a tolerant delete now treats an absent *container* (bucket, Azure container) as an absent path on every flat-namespace backend: `delete(path, missing_ok=True)` and `delete_folder(path, missing_ok=True)` both return cleanly, and both raise `NotFound` without `missing_ok`. Previously the two disagreed against the same missing bucket, because a bodyless `HeadObject` 404 cannot name the bucket while a `ListObjectsV2` 404 can. Affects `delete_folder` on `S3Boto3Backend`, `AzureBackend` and `AsyncAzureBackend` (which raised); `SQLBlobBackend` is unaffected — it settles its table's existence in `__init__`, so a dropped table stays `BackendUnavailable`
