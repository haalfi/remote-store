...
- BK-357: **Breaking** — seeking to the end of an `SFTPBackend` stream on a stalled connection now raises instead of answering `0`. `SFTPFile.seek(offset, SEEK_END)` resolves the position through paramiko's `_get_size()`, whose whole body is `try: return self.stat().st_size` under a bare `except: return 0`, so the size request blocked for `io_timeout` and was then discarded: the seek *answered* `0` on a file of any size and raised nothing. Three consequences followed, and the first is the one a caller could act on wrongly — the answer was indistinguishable from a genuinely empty file, so code sizing a file by seeking to its end read zero bytes with nothing to catch; nothing reached the wrapper's mapper, so BK-355's futile-close guard stayed unarmed and the close paid the bound a second time; and the dead client stayed cached, so the next operation re-entered the same channel and waited again. `_ErrorMappingStream` now takes an optional `size_probe` and `SFTPBackend.read()` supplies one that issues `CMD_FSTAT` against the open handle, letting the failure travel the ordinary mapping path; the wrapper then delegates an absolute seek, which is local on a paramiko handle rather than a second round-trip. Measured at a 2 s `io_timeout`, consuming part of a `read()` and then stalling: 4.00 s answering `0` on a 1 MiB file with the client still cached, against 2.00 s raising `BackendUnavailable` with the client dropped. **Upgrade note:** a `SEEK_END` seek that a stalled or otherwise failing size request used to answer now raises. This reaches callers who never wrote a seek — `SFTPFile.seekable()` is `True` unconditionally, so `read_seekable()` hands the stream straight to analytical readers such as PyArrow, which size a file with an end-relative seek before reading its footer — exercised end to end by `TestParquetOverSftpReachesTheSizeProbe`. That applies to reads large enough to stream: `ext.arrow` materialises anything at or below its `materialization_threshold` and never seeks the stream. Code that treated a `0` from seek-to-end as "empty file" was reading a wrong answer and should now catch `RemoteStoreError`, which covers both causes: a stalled connection raises `BackendUnavailable` and drops the cached client, while a server refusing to stat the handle raises the generic `RemoteStoreError`, because that connection is healthy and only the one request was refused. A server that answers `SSH_FX_OP_UNSUPPORTED` to `CMD_FSTAT` is affected the same way and without any stall: it used to get a silent seek-to-0 on a perfectly readable file, and now gets an error — pinned by `test_seek_to_end_raises_when_the_server_refuses_to_size_the_handle`, which drives such a server and asserts both the new error and paramiko's old swallow. `read_bytes` already failed against such a server (paramiko's prefetch stats the handle), so this makes the two consistent rather than newly restricting anything. Upgrade path in the [migration guide](https://docs.remotestore.dev/stable/reference/migration/#v0300-to-v0310). **`S3Backend`, `S3Boto3Backend`, `S3PyArrowBackend`, `AzureBackend` and `ReadOnlyHttpBackend` are unaffected**: they supply no probe, so their seek path is byte-for-byte what it was. The hook is opt-in for the same reason `is_fatal` is — paramiko is the only inner stream measured to discard its own size failure, and every other stream the wrapper serves reaches `SEEK_END` by a route with nothing for a probe to repair: `S3Boto3Backend`'s ranged reader (both its `read` and its `read_seekable`) and `AzureBackend.read_seekable()`'s add `offset` to a size captured at open, `S3Backend`'s fsspec handle does the same with a size it holds, `S3PyArrowBackend` defers to a pyarrow `NativeFile`, and the forward-only adapters behind `AzureBackend.read()` and `ReadOnlyHttpBackend.read()` are not seekable at all. Probing on their behalf would buy a round-trip per seek and nothing else. SFTP-030 consequently states **one** exception rather than two, the `subsystem` request, which is the only genuinely unbounded wait left; the new clause is SIO-011. Not closed by this: a `paramiko.SFTPError` raised by the probe still escapes the wrapper unmapped, exactly as one raised by a read does, because the caught tuple is unchanged — that is BK-358
- BK-355: **Fix** — releasing a stream whose read failed on a stalled connection no longer pays the bound a second time. `_ErrorMappingStream.close` closed the underlying handle unconditionally, and on `SFTPBackend` that close re-entered the channel the read failure had just condemned: paramiko's `SFTPFile.close()` issues a synchronous `CMD_CLOSE` and waits for a reply that never comes, so a caller consuming a stalled `read()` waited `io_timeout` twice — silently, since paramiko swallows the timeout raised inside its own close and the wrapper suppressed what was left. The shared wrapper now takes an optional `is_fatal` predicate and skips a close that would re-enter a connection its own failure condemned (SIO-010); `SFTPBackend` supplies `_is_connection_dead`, which is the rule its `_handle` guard already applies to every handle the backend itself owns, extended to the one handle the wrapper owns instead. Measured at a 2 s `io_timeout`, consuming part of a `read()` and then stalling: 4.00 s before, 2.00 s after. The handle is not leaked — it is released by the peer's teardown of the dead connection or at collection, where paramiko's `SFTPFile.__del__` sends `CMD_CLOSE` without waiting — so what changes is a synchronous release that could not succeed becoming an asynchronous one that costs the caller nothing. **`S3Backend`, `S3Boto3Backend`, `S3PyArrowBackend`, `AzureBackend` and `ReadOnlyHttpBackend` are unaffected**: they supply no predicate and their stream close is unchanged. The guard is deliberately opt-in rather than derived from the error classification, because `ReadOnlyHttpBackend` maps every stream exception to `BackendUnavailable` and a classification-driven rule would have stopped it releasing response bodies on an ordinary read error. It did not reach a `SEEK_END` seek when it landed: paramiko's `SFTPFile._get_size` swallows the stalled `stat` under a bare `except`, so nothing reached the wrapper and the close paid the bound again — measured at 4.00 s and stated as an exception in SFTP-030. BK-357 closes that in this same release by having the wrapper issue the size request itself, so SFTP-030 now records one exception rather than two: the `subsystem` request, the only genuinely unbounded wait left
- BK-354: **Added** — `SFTPBackend(io_timeout=...)` bounds a read or write that stalls on an already-open channel. `timeout` covers only the connect phase (it is passed as paramiko's `timeout` / `banner_timeout` / `auth_timeout` / `channel_timeout`, the last of which bounds channel *open*), and paramiko leaves an opened channel unbounded, so a peer that finishes the handshake and then stops sending previously blocked forever with no error. `io_timeout` bounds the silence between bytes rather than the transfer as a whole, so a large file over a slow link is unaffected; a stall raises `BackendUnavailable` and clears the cached client so the next operation reconnects. It is armed on every connect **and every reconnect**, which a caller cannot arrange from outside — a `settimeout()` on an unwrapped client is lost with the channel it was set on — and it is armed before the SFTP version exchange, so a peer that completes the SSH handshake and then falls silent during session setup is bounded too rather than hanging. The bound is what the caller waits, once: no operation re-probes or tears down a channel already known to be dead, which would otherwise re-pay the timeout on each classification step, on the rename fallbacks in both `write_atomic`/`open_atomic` and `move`, and on the close of every file handle a read, write, copy or move holds open — the last of those silently, since paramiko suppresses the timeout raised inside its own close. `copy` is the worst case, holding two handles at once: measured at a 2 s bound, a copy stalled mid-transfer took 6.9 s before the guard and 2.0 s after. SFTP-030 states its exceptions rather than assuming them away, and the two open when this landed were different in kind: the SSH `subsystem` request is genuinely **unbounded**, since paramiko waits for it on an untimed event; releasing a stalled stream handle was **bounded but paid twice**, and is closed by BK-355 in this same release. A `SEEK_END` seek — bounded, paid twice, *and answered wrongly*, because paramiko swallows the stalled `stat` — took its place in the list and is closed by BK-357, also in this release, leaving the `subsystem` request as the only one. As a side effect, `_has_file_ancestor` no longer swallows a dead-connection stat, so an operation that dies mid-classification now reports `BackendUnavailable` and clears the client instead of degrading to a generic `RemoteStoreError` that left the dead client cached. A streamed read raises rather than returning short, so a truncated transfer is never mistaken for a complete one. Default is `None` (unbounded), so no existing caller changes behaviour; `0` and negative values raise `ValueError`. No stall is retried — `RetryPolicy` wraps the SSH connect call alone — so a partially consumed stream is never silently restarted
- BUG-259: **Fix** — a write to the store root is now refused before anything reaches the storage system, on every backend that declares `WRITE`. `SFTPBackend` was the damaging case: with `base_path` absent, `write("")`, `write_atomic("")` and `open_atomic("")` ran to completion against the container path itself, so the store's own container was left a regular **file** — `open_atomic` returned cleanly having done it, and the other two raised `InvalidPath` only from the path layer *above* the backend, after the bytes. Both spellings (`""` and `"."`) and both overwrite modes were affected. Five more classes reached their SDK with the root key and answered with the wrong error rather than corrupting anything: `S3Backend` raised `AlreadyExists` (asserting an object that does not exist) without `overwrite` and leaked a raw parameter-validation error with it; `S3Boto3Backend` raised `BackendUnavailable` — a retryable class for a permanently wrong request — for `""` while `"."` took a different route entirely, so the two spellings disagreed; `S3PyArrowBackend`, `AzureBackend` and `AsyncAzureBackend` surfaced the SDK's own wording unclassified. All six now raise `InvalidPath` naming the root, decided from the key with no round trip. **The `move`/`copy` destination is refused on the same terms**, which turned out to matter most on the lanes where it looked least necessary: `S3Boto3Backend.move(src, ".")` **returned cleanly and deleted the source file**, and `S3Backend` answered `AlreadyExists` for a destination that does not exist. A caller who names the root as a `move` destination now gets `InvalidPath` before any round trip, in place of silent data loss — and gets it ahead of the source-existence check, so the answer no longer depends on whether their source happened to exist. A write to a path *under* the root is unaffected and still creates the container where that is the backend's documented behaviour. `GraphBackend` gains the destination guard too, and a related ordering fix: a write to the drive root on a **closed** backend raised `InvalidPath` ("cannot write to the drive root") where the contract requires `BackendUnavailable` — the root check ran ahead of the closed-state guard, so a caller could not tell a closed store from a bad path. **The refusal also now covers spellings beyond `""` and `"."`**: `"./"` addresses the same node and was accepted by every guard, so on a `LocalBackend` whose root had been deleted `write("./")` left the root a regular file and `open_atomic("./")` returned cleanly having done it. The write guard decides root-ness on the normalised key, so `"./"`, `".//"`, `"./."` and `"/"` are all refused; the shared `is_root` predicate is unchanged, and reads, probes, addressing and listings keep today's behaviour. `GraphBackend` also refuses the root as a `move`/`copy` **source** from the key: it previously fetched the item and read the verdict off the response, so a root source cost a network round trip and answered after it. Two further `GraphBackend` corrections fall out of the same rule. A **closed** backend handed `metadata=` answered `CapabilityNotSupported` rather than `BackendUnavailable`, because the user-metadata gate ran ahead of the closed-state check just as the root check once did; the closed check now runs at the top of `write`, ahead of every pre-check, so a caller holding a closed store is told that whatever else is also wrong with the call. And a `move`/`copy` **destination** spelled with a dot segment — `"./x.txt"`, `".//x.txt"` — passed the destination guard and was then addressed as a folder literally named `.` on the wire, while `write("./x.txt")` wrote to the drive root: the destination address carried its own segment predicate that kept `"."` where every other one drops it. Both the parent and the name halves of the destination address now use the same predicate as the guard and as `native_path`, so which node a key names and whether it names one are decided the same way — `move(src, "a/.")` likewise addressed an item literally named `.` inside `a` while `write("a/.")` wrote to `a`. **`GraphBackend(base_path=...)` is normalised by that predicate too, which changes what one value means**: `base_path="."` previously scoped the entire store under a drive folder literally named `.` and sent every write there, and now targets the drive root, as `"."` does everywhere else in the contract. `"./"`, `".//"` and any interior `"."` segment normalise the same way, so `base_path="a/./b"` scopes to `a/b`. The cost is that a drive folder genuinely named `.` can no longer be named by `base_path` — it remains reachable as an ordinary key under a `base_path` that is not itself a root spelling. Every other `base_path` value is unaffected. Conformance gains cells holding every `WRITE`-capable backend to both halves of the root rule in **both overwrite modes**, and to that ordering on the write path
- BK-355: **Fix** — releasing a stream whose read failed on a stalled connection no longer pays the bound a second time. `_ErrorMappingStream.close` closed the underlying handle unconditionally, and on `SFTPBackend` that close re-entered the channel the read failure had just condemned: paramiko's `SFTPFile.close()` issues a synchronous `CMD_CLOSE` and waits for a reply that never comes, so a caller consuming a stalled `read()` waited `io_timeout` twice — silently, since paramiko swallows the timeout raised inside its own close and the wrapper suppressed what was left. The shared wrapper now takes an optional `is_fatal` predicate and skips a close that would re-enter a connection its own failure condemned (SIO-010); `SFTPBackend` supplies `_is_connection_dead`, which is the rule its `_handle` guard already applies to every handle the backend itself owns, extended to the one handle the wrapper owns instead. Measured at a 2 s `io_timeout`, consuming part of a `read()` and then stalling: 4.00 s before, 2.00 s after. The handle is not leaked — it is released by the peer's teardown of the dead connection or at collection, where paramiko's `SFTPFile.__del__` sends `CMD_CLOSE` without waiting — so what changes is a synchronous release that could not succeed becoming an asynchronous one that costs the caller nothing. **`S3Backend`, `S3Boto3Backend`, `S3PyArrowBackend`, `AzureBackend` and `ReadOnlyHttpBackend` are unaffected**: they supply no predicate and their stream close is unchanged. The guard is deliberately opt-in rather than derived from the error classification, because `ReadOnlyHttpBackend` maps every stream exception to `BackendUnavailable` and a classification-driven rule would have stopped it releasing response bodies on an ordinary read error. It does not reach a `SEEK_END` seek: paramiko's `SFTPFile._get_size` swallows the stalled `stat` under a bare `except`, so nothing reaches the wrapper and the close pays the bound again — measured at 4.00 s, stated as an exception in SFTP-030 and filed as BK-357, since fixing it means the wrapper issuing its own size probe on every backend's seek path. SFTP-030 therefore still records two exceptions: that seek, which is bounded but paid twice, and the `subsystem` request, which is the only genuinely unbounded wait left
- BK-354: **Added** — `SFTPBackend(io_timeout=...)` bounds a read or write that stalls on an already-open channel. `timeout` covers only the connect phase (it is passed as paramiko's `timeout` / `banner_timeout` / `auth_timeout` / `channel_timeout`, the last of which bounds channel *open*), and paramiko leaves an opened channel unbounded, so a peer that finishes the handshake and then stops sending previously blocked forever with no error. `io_timeout` bounds the silence between bytes rather than the transfer as a whole, so a large file over a slow link is unaffected; a stall raises `BackendUnavailable` and clears the cached client so the next operation reconnects. It is armed on every connect **and every reconnect**, which a caller cannot arrange from outside — a `settimeout()` on an unwrapped client is lost with the channel it was set on — and it is armed before the SFTP version exchange, so a peer that completes the SSH handshake and then falls silent during session setup is bounded too rather than hanging. The bound is what the caller waits, once: no operation re-probes or tears down a channel already known to be dead, which would otherwise re-pay the timeout on each classification step, on the rename fallbacks in both `write_atomic`/`open_atomic` and `move`, and on the close of every file handle a read, write, copy or move holds open — the last of those silently, since paramiko suppresses the timeout raised inside its own close. `copy` is the worst case, holding two handles at once: measured at a 2 s bound, a copy stalled mid-transfer took 6.9 s before the guard and 2.0 s after. SFTP-030 states two exceptions rather than assuming them away, and they are different in kind: the SSH `subsystem` request is genuinely **unbounded**, since paramiko waits for it on an untimed event; a `SEEK_END` seek is **bounded but paid twice and answered wrongly**, because paramiko swallows the stalled `stat` so nothing reports the failure or clears the cached client. Releasing a stalled stream handle was the second of the two stated when this landed, and is closed by BK-355 in this same release; the `SEEK_END` case took its place in the list. As a side effect, `_has_file_ancestor` no longer swallows a dead-connection stat, so an operation that dies mid-classification now reports `BackendUnavailable` and clears the client instead of degrading to a generic `RemoteStoreError` that left the dead client cached. A streamed read raises rather than returning short, so a truncated transfer is never mistaken for a complete one. Default is `None` (unbounded), so no existing caller changes behaviour; `0` and negative values raise `ValueError`. No stall is retried — `RetryPolicy` wraps the SSH connect call alone — so a partially consumed stream is never silently restarted
- BUG-247: **Fix** — `LocalBackend` no longer answers every operation with `InvalidPath("Path escapes root directory")` once its root directory is deleted underneath it. Its containment check walked up to the deepest existing ancestor for a symlink-escape test, which climbs past the root once the root is gone, so absence was reported as an escape — including from `delete(missing_ok=True)` and `delete_folder(missing_ok=True)`, which the backend contract requires to return cleanly, and from `exists()`, which it forbids from raising at all. The walk now stops at the root. An absent root reads as an absent store throughout: the tolerant deletes return, the strict deletes and `read`, `read_bytes`, `read_seekable`, `get_file_info`, `get_folder_info` and the `move`/`copy` source raise `NotFound`, the three probes answer `False`, and every listing is empty. A path that genuinely points outside the root is still `InvalidPath`. The store root itself keeps its contract answers — `exists("")` and `is_folder("")` are `True`, `get_folder_info("")` aggregates to zero, and a file-shaped call on the root is `InvalidPath` with or without `missing_ok` — which Local previously got right only because its root existed; `check_health()` remains the operation that reports an absent root, and now also reports a root path occupied by something that is not a directory — it tests `is_dir()` rather than mere existence, because the probes above answer the root by definition and no longer observe it. A `write()` to a path *under* an absent root recreates the root and succeeds, as construction always has; a write to the root key itself is now refused up front with `InvalidPath` rather than by observing a directory — with the root gone there was no directory to observe, and the write ran to completion, leaving the store root as a regular file
...
- BUG-248: **Breaking** — `GraphBackend` (and its sync adapter) now reads an absent drive as an absent path on every operation the backend contract decides, where it previously raised `BackendUnavailable` whenever Graph reported `404 resourceNotFound`. `delete(missing_ok=True)` and `delete_folder(missing_ok=True)` return cleanly; the strict deletes, `read`, `read_bytes`, `get_file_info`, `get_folder_info` and the `move`/`copy` source raise `NotFound`; `list_files`, `list_folders` and `iter_children` yield nothing; `exists`, `is_file` and `is_folder` still answer `False`. Four callers keep the old escalation, because the contract does not decide them: `write` still raises `BackendUnavailable` on a drive-identity 404 (so a misconfigured drive surfaces as a configuration error rather than as a missing file), and so do `check_health`, `GraphUtils.resolve_drive_id` and the copy/move monitor poller. Resolves a contradiction between BE-021 and GR-031 in which both clauses were deliberate; see ADR-0038. A caller that distinguished a dead drive from a missing item by catching `BackendUnavailable` on a read must now use `check_health` or `write` — and note both escalate only when Graph names the drive with `error.code == "resourceNotFound"`, which some tiers (consumer OneDrive among them) never emit for a dead drive, in which case no operation distinguishes the two. The drive-root probe `exists("")` GR-031 documents works **only on a store with no `base_path`**; on a scoped store it answers `False` for a missing `base_path` folder on a perfectly reachable drive
- BUG-243: **Change** — a tolerant delete now treats an absent *container* (bucket, Azure container, SQL table) as an absent path: `delete(path, missing_ok=True)` and `delete_folder(path, missing_ok=True)` both return cleanly, and both raise `NotFound` without `missing_ok`. Previously the two disagreed against the same missing bucket, because a bodyless `HeadObject` 404 cannot name the bucket while a `ListObjectsV2` 404 can. Affects `delete_folder` on `S3Boto3Backend`, `AzureBackend` and `AsyncAzureBackend` (which raised `NotFound`), and both deletes on `SQLBlobBackend` against a dropped table (which raised `BackendUnavailable`). **On `SQLBlobBackend` this also changes use-after-close for an in-memory store:** disposing an in-memory SQLite engine destroys the database rather than releasing a connection to it, so after `close()` both tolerant deletes now return silently where they previously raised `BackendUnavailable`. `read` and the probes still raised at the time, so a discarded in-memory store answered inconsistently; BUG-246 in this same release closes that asymmetry, and every operation now reports an empty store. A caller who relied on a tolerant delete raising after `close()` still needs to track the closed state itself. Two backends still answered differently when this landed and are resolved in this same release: `LocalBackend` with a deleted root, by BUG-247, and `GraphBackend` with an absent drive, by BUG-248
