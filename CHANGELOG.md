...
- ID-158: Fix `ResourceWarning` from phantom event loop leaked by pytest-asyncio 1.3 on Python 3.11 — `asyncio.get_event_loop()` auto-creates a loop when none is set; pytest-asyncio saves it as `old_loop` and restores it as the policy default after every async test, so `asyncio.run()` in a subsequent sync test orphans it; the loop's internal cyclic reference (`_read_from_self` ↔ Handle) keeps it alive until `gc.collect()` at session teardown where `BaseEventLoop.__del__` emits `ResourceWarning` (promoted to error by `filterwarnings = error`). Fix: session-scoped autouse fixture in `tests/conftest.py` closes all unclosed non-running event loops before pytest's `gc_collect_harder()` runs. Also: restore the `os_sensitive` marker on the `adapter-local` param of `adapted_backend` in `tests/aio/test_sync_adapter_conformance.py` (unblocked by the fix) so cross-platform CI exercises `SyncBackendAdapter(LocalBackend(...))` conformance on Windows and macOS.
