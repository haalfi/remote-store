...
- ID-160: `context7.json` — shorten rules 3, 6, 8 to ≤255 chars (Context7 schema limit). Refresh of `/haalfi/remote-store` was failing validation: `rules.3: String exceeds maximum length of 255; rules.6: ...; rules.8: ...`. Content semantics preserved. Add three rules (9–11) covering `WriteResult` shape + `source` flavours, `metadata=` kwarg + `USER_METADATA` gate, and the capability-flavour split (method gates vs quality flags) so LLM clients see them as priors. Add `sdd/specs/` to indexed `folders` (46 spec files with formal pre/postconditions and worked examples for each Store method, backend, error, capability, and ext module); drop the now-redundant `sdd/` entry from `excludeFolders`. Other `sdd/` subdirs (process, backlog, ADRs, RFCs, audits, research, formal) remain unindexed by virtue of not being in `folders`. Move five standalone prose pages from `docs-src/` to `guides/` to align with the established convention (`docs-src/<page>.md` is a 4-line `include-markdown` stub; canonical prose lives in `guides/`): `architecture.md`, `security-model.md`, `capabilities-matrix.md`, `write-integrity.md`, `further-reading.md`. mkdocs nav and `_link_map.yml` unaffected (titles stay the same; `docs-src/` stubs preserve URLs). Context7 picks up the moved files automatically via `guides/`. `mkdocs build --strict` clean. Diátaxis-aligned filesystem reorg tracked separately as ID-174.
- ID-174: backlog item filed — Diátaxis-aligned docs filesystem reorg under a single `docs/` umbrella (`docs/{tutorials,how-to,reference,explanation,further}/`); `guides/` disappears, `FEATURES.md` and `migration.md` move into `docs/reference/`. Sequenced after ID-161 (`llms.txt`).
