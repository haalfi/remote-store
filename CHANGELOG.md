...
- **Dafny `WriteResult` extension** (ID-151, part 1): widens the Backend
  ...
  and the `WriteResultFromFileInfo` function plus `WR008FieldMapping` /
  `WR006SidecarProvenance` lemmas. Encodes WR-001a, WR-004, WR-005,
  WR-010 (with the empty-mapping carve-out via `HasUserMetadata`),
  WR-012, and WR-013 as Write postconditions; `MemoryBackend` refines
  the widened contract. Oracle regeneration, `DafnyOracleBackend`
  adapter, and the oracle-gated conformance run are follow-up scope
  (Dafny-native toolchain required; local is Docker-only).
