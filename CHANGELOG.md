...
- BUG-274: **Fix** — an SFTP operation against a host that was never reached pays the connect retry budget once, not two or three times: a `read_bytes` against a refused port drops from 8.00 s to 4.00 s. A probe that reconnects into a gone host now reports `BackendUnavailable` and clears the cached client
