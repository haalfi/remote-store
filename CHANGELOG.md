...
- BK-174: document `InvalidPath` on async `write`/`write_atomic` across the `AsyncBackend` ABC, `AsyncAzureBackend`, and `SyncBackendAdapter`; bundle the matching `--` → `—` swap in `AsyncBackend.delete_folder` and add `write_atomic(dir)` regression coverage (ASYNC-010) to the async conformance suite.
