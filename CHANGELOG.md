...
- BK-227: `Store.move` / `copy` and `AsyncStore.move` / `copy` now raise `InvalidPath` (was `NotFound`) when the source path is a directory and `src == dst`; the self-op short-circuit probes `is_file` first (1 RTT for the common no-op case), then `is_folder` to distinguish a directory source from a missing source (BE-018, BE-019, BE-021)
