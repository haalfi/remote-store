...
- BK-360: Document what a stalled non-atomic SFTP `write` (and `copy`) leaves at the destination: absent, empty, or an unpredictable prefix, so a retry needs `overwrite=True` and a failure is never a no-op
