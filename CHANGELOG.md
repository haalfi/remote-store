...
- ID-198: Fix the Medallion + Dagster showcase — runs on current Dagster again, the lake is OTel-observed so a backend swap keeps emitting spans, and the Azure/HNS one-line swap is documented; verified end-to-end against live ADLS Gen2 and guarded by an offline smoke test
