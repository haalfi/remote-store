...
- BUG-195: `AzureBackend.get_file_info` and `AsyncAzureBackend.get_file_info` raise `InvalidPath` (not `NotFound`) when the path names an HNS directory blob (`hdi_isfolder=true`); the three `TestAzureLiveHnsGetFileInfoOnDirectory` / `TestAsyncLiveHnsGetFileInfoOnDirectory` live tests now assert the spec instead of documenting the deviation (BE-016, ASYNC-016, BE-021)
- BUG-203: `AzureBackend.is_folder()` returns `False` (not `True`) for HNS file paths; extends the `hdi_isfolder` probe used by `write`/`write_atomic`/`open_atomic` to the `is_folder` HNS branch. Async sibling unaffected (no `[azure_live_async]` failure for this test) (BE-005, BE-021)
