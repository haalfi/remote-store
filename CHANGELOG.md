...
- BUG-281: A SQLite URL spelling an in-memory database with `mode=memory` now states its connection pool rather than letting SQLAlchemy infer one, so constructing `SQLBlobBackend` or `SQLQueryBackend` on it no longer warns under SQLAlchemy 2.1 and the URL keeps working across threads.
