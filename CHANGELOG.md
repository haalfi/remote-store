...
- **`hatch run notebooks` smoke-test runner** (ID-048) -- lightweight script (`tests/run_notebooks.py`) that executes tutorial notebook code cells via `exec()` without requiring Jupyter. Wired into `hatch run all` for CI coverage. Skips `benchmark_analysis.ipynb` (needs pre-generated data).
