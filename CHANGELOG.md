...
- BUG-246: **Breaking** — an absent container (a missing S3 bucket, a missing Azure container, a dropped SQL table) now reads as an absent path on the four backends that raised instead. On `S3Boto3Backend`, `AzureBackend` and `AsyncAzureBackend`, `exists` and `is_folder` answer `False` where they raised `NotFound` (`is_file` already answered `False` — its HEAD absorbed the 404, which is why only the prefix-listing-backed probes diverged), and on the two Azure adapters `list_files`, `list_folders`, `iter_children` and `glob` now come back empty where they raised `NotFound`. That tolerance is bounded to the first page on both adapters, as on `S3Boto3Backend`: a container deleted *between pages* of a listing still raises, so a mid-scan deletion is not reported as a complete listing. The bound triggers on a **page** having come back rather than on an item having been yielded, which matters because every listing filters part of a page away — a container of folders with no top-level blobs hands `list_files` a first page it discards entirely, and the container plainly existed. On the two Azure adapters the HNS (ADLS Gen2) branches carry the same bound, executed against a wire stub rather than argued from the code. On `SQLBlobBackend` every operation raised `BackendUnavailable`, so all three probes now answer `False`, `read`, `read_bytes`, `get_file_info`, `get_folder_info` and the `move`/`copy` source raise `NotFound`, and the four listings come back empty. **The store root is the exception on both counts**, because BE-029 makes it a folder that always exists and outranks the row above: against a dropped table `exists("")` and `is_folder("")` still answer `True`, and `get_folder_info("")` returns `FolderInfo(file_count=0)` rather than raising — an absent store and an empty one agree there, and only there. `SQLBlobBackend.write` is the one operation deliberately unchanged and still raises `BackendUnavailable`: BE-021 declines to decide what a write owes an absent container, so a vanished table stays a configuration failure from a writer's point of view. **On `SQLBlobBackend` this also completes the use-after-close change BUG-243 started for an in-memory store:** disposing an in-memory SQLite engine destroys the database, so after `close()` every operation above now reports an *empty* store rather than the deletes returning while everything else raised. A caller that distinguished a dead store from an empty one by catching `BackendUnavailable` on a read must now know that there is **no portable replacement**, and that the obvious candidates are worse than they look. `check_health()` is not one: measured against an absent container it raises on `S3Backend`, `S3Boto3Backend`, `AzureBackend`, `AsyncAzureBackend` and `LocalBackend` but returns cleanly on `S3PyArrowBackend`, `SQLBlobBackend` and `SQLQueryBackend`, because on those it probes the service rather than the container (tracked as BUG-256). A trial `write()` does distinguish the two cases where writes are permitted at all — measured, it raises on every backend against an absent container — but it leaves an object behind, raises `CapabilityNotSupported` on read-only backends regardless of the container, raises `AlreadyExists` on the second call without `overwrite=True`, and raises `PermissionDenied` when the store is present but unwritable, so catching `RemoteStoreError` around it reports "gone" for a healthy store. Pick a probe that fits your backend and test it against a container you have really deleted. `S3Backend` and `S3PyArrowBackend` already answered this way and are unchanged; `LocalBackend` with a deleted root still raises `InvalidPath` and is tracked separately — BE-021 lists it as the last of its known divergences
- BUG-249: **Breaking** — `S3Boto3Backend.list_files`, `.list_folders` and `.iter_children` no longer let a raw `botocore.exceptions.ClientError` escape. They were the only three methods on the class whose wire call was not wrapped in its error mapper, so a caller writing `except RemoteStoreError` caught every backend but this one, and the escaping type came from a library they may never have imported. All three now map their errors like the rest of the class — a denial raises `PermissionDenied`, throttling and 5xx raise `BackendUnavailable` — and an absent bucket yields an empty listing rather than raising, matching the two s3fs-backed lanes against the identical response. `glob` reaches the wire only through `list_files` and gets both changes with it. A caller catching `ClientError` around these three listings must switch to `RemoteStoreError`. The absent-bucket tolerance is **bounded to the first page**: a bucket that is absent when the listing starts yields nothing, but one deleted *between pages* raises rather than returning a short listing that looks complete. The bound keys on a page having come back, not on an item having been yielded — a first page whose contents this listing filters away still proves the bucket exists. `S3Backend` and `S3PyArrowBackend` still truncate silently in that case, which is pre-existing and tracked as BUG-255
