...
- ID-227 (phase 1): Add generated per-backend **retryable-vs-terminal** and **write/move atomicity** matrices to `FEATURES.md` (and thus the `llms-full.txt` bundle). The retry status split is projected drift-proof from `_retry.py`'s `RETRYABLE_STATUSES` / `TERMINAL_STATUSES`, and each `move` atomicity cell is cross-checked against the documentation graph's `ATOMIC_MOVE` capability, so the bundle now carries the cross-backend guarantee contract no single docstring holds. Consistency (phase 2) and cost (phase 3) remain open.
