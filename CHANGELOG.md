...
- BUG-281: A SQLite URL spelling an in-memory database with `mode=memory` now states its connection pool instead of letting SQLAlchemy infer one, so `SQLBlobBackend` and `SQLQueryBackend` no longer warn under SQLAlchemy 2.1; which pool follows `cache=shared`, and each spelling keeps the sharing it had.
