...
- BK-356: **Breaking** — SFTP reads and writes are bounded by default: `io_timeout` defaults to 120 s rather than being opt-in, so an unconfigured store no longer hangs on a silent peer
