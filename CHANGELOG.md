...
- BUG-275: **Fix** — an SFTP failure denied with `EPERM` raises `PermissionDenied` on every operation, where only `EACCES` did; `SFTPBackend` and `LocalBackend` now answer a denial alike. Upgrade path in the [migration guide](https://docs.remotestore.dev/stable/reference/migration/#v0300-to-v0310).
