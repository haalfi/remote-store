...
- BUG-193: Completed the async HNS live test gap deferred by BUG-182: replaced the broken `TestAsyncAzureLiveHNS` (used the Azurite-backed fixture; `_ensure_hns()` returned `False` on Azurite so the HNS rename path was never exercised) with `tests/aio/test_async_azure_live_hns.py` — a dedicated file with a real-ADLS-Gen2 fixture and no Azurite dependency. Adds `TestAsyncLiveHnsWriteResult` (WR-001a/WR-004/AZ-034: source, size, etag normalisation, last_modified tz-awareness, etag cross-check vs `get_file_info`), `TestAsyncLiveHnsMetadata` (WR-012 echo + WR-013 rename survival), and `TestAsyncLiveHnsDirectoryGuard` (`write` and `write_atomic` on a real HNS directory blob; BE-021, BE-008, BE-010). Sync `test_azure_live_hns.py` also gains `WriteResult` field assertions on the existing metadata test (WR-012 echo, WR-001a size/source) and a new `TestAzureLiveHnsWriteResult` class that cross-checks `WriteResult.etag` against `get_file_info().etag` — the only assertion that can catch normalisation drift between the post-rename `get_file_properties` read and the independent `get_file_info` read on a real account (WR-001a, WR-004, AZ-034).
