...
- BK-227: `Store.move` / `copy` and `AsyncStore.move` / `copy` now raise `InvalidPath` (not `NotFound`) when the source path is a directory and `src == dst`; the `is_folder` probe precedes the `is_file` check in the self-op short-circuit (BE-018, BE-019, BE-021)
