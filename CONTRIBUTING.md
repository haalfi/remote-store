...
- [ ] `sdd/BACKLOG-DONE.md`: all shipping items moved here under the `## Unreleased` heading, each marked `[x]` (Phase 2 versions the heading). **First in this phase deliberately:** two bullets below read this section — *`[Unreleased]` is complete*, whose gate compares against it, and *`[Unreleased]` condensed*, which takes each entry's prose from the item bodies in it. Sweep after them and those two run against a section that is not yet complete. (The *CHANGELOG completeness cross-check* between them does not read this section; it reads the generate-notes output and the CHANGELOG.)
