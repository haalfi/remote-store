...
- [ ] Support windows checked against [Rules 8 and 9](docs-src/explanation/dependency-policy.md#rule-8), which now have mechanisms behind them rather than this line's memory:
  ...
  - **Rule 8, the interpreter window:** open the chart on the [dependency policy page](docs-src/explanation/dependency-policy.md#rule-8). Each bar runs from a version's release to the end of its security support and the vertical line is today, both drawn when the page is viewed, so it is current whenever you look — which is why it rather than the issue is this step's surface. **A crossing is surfaced on the rolling `[drift-guard]` issue**, whose **Support windows** section carries the same rows with days remaining: a version past its window with no row in `infra/drift-locks/PYTHON-SUPPORT.md` holds that issue open, and is a decision this release either takes or registers. **So a closed issue is itself the answer** that nothing has crossed — the section is only written when the run has a body to write, so do not read a missing one as a missing check, or an old one as today's. **Advisory either way:** a closed window licenses a drop, it never requires one — nothing is broken on the day one closes
