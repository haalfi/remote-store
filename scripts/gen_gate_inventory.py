...
_BOUNDS: tuple[str, ...] = (
    ...
    "`python -c` step in the `drift-smoke` composite action; `gen_pages.py`, under "
    "`mkdocs.yml`'s gen-files plugin and so run by `docs-build`; and "
    "`check_docs_framework.py`'s G-05, which imports `docs.check_links`. The last "
    "is a genuine pair gate, so this escape can hide one — it is listed here only "
    "because `check_links.py` is separately wired as a command. "
    "`drift_check.py`'s `_direct_requirements_for` was a fourth until the import "
    ...
    "it is named — a second shape of the escape above. **`scripts/import_probe.py` "
    "is the live instance**: `.github/actions/drift-smoke/action.yml` runs it by "
    "path on every leg of both lanes, and no row here says so. An import would be "
    "equally invisible, so the action's other two reaches — `drift_smoke_map` and "
    "`drift_check` — are not a mitigation of this bound; it consumes their return "
    "values in shell, which is why those are imports.",
    ...
    "are none of them — a third escape beside the two above, and the only one "
    "whose home is a document rather than a file a gate executes. "
    "**`scripts/check_backlog_ids_vs_base.py` is the live instance**: it is "
    "mandatory at PR time and its row says `advisory`, because its only pyproject "
    "home is a bare alias. It is wired that way on purpose — it compares against a "
    "base ref on the remote and so needs that ref fetched, which a bundle entry "
    "cannot guarantee and CI's checkout does not supply — so the under-statement "
    "is the price of the wiring rather than a mistake in it. This is the sharpest "
    "form of the bound below: *Enforcement is derived from wiring, not exit "
    "codes*, and a home outside the three sources is no wiring at all.",
