...
_BOUNDS: tuple[str, ...] = (
    ...
    "it is named. Four live instances: `drift_smoke_map.py`, imported by a "
    "`python -c` step in the `drift-smoke` composite action, and "
    "`drift_check.py`'s `_direct_requirements_for`, imported by a `python -` "
    "here-document in the same action; `gen_pages.py`, under `mkdocs.yml`'s "
    "gen-files plugin and so run by `docs-build`; and `check_docs_framework.py`'s "
    "G-05, which imports `docs.check_links`. The last is a genuine pair gate, so "
