...
def render(mechanisms: list[Mechanism]) -> str:
    ...
    out: list[str] = [
        ...
        "<!-- doc: repo-only -->",
        "",
        # No grand total here, deliberately. It was `Derived from N declared
        # mechanism(s)`, and N is exactly the sum of the three section
        # headings below -- a reader who wants it can add three numbers that
        # are already on the page. What it did do was invite copies: it is the
        # figure prose elsewhere quotes, and those copies do go stale, since
        # only this file is regenerated. `--check` keeps this file honest and
        # nothing keeps a trace or a backlog entry honest. The per-kind counts
        # stay because each is checkable against the table beneath it, and the
        # CLI still prints the total, where it is feedback rather than a
        # durable claim.
    ...
    out += [
        "",
        f"## What this inventory does not catch ({len(_BOUNDS)})",
        "",
