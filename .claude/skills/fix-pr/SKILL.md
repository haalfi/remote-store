...
(`hatch run all` for code changes incl. `scripts/`, else `hatch run lint` +
`docs-gate`), local-machine reference, qualitative TESTING/CONTENT review — with
`<BASE>` = `master`. Report violations and resolve any stop condition before
committing.
