...
root: every local `Read`, `Grep` and `Glob` reads there, `git` takes
`-C <path>`, and before the first gate or `python` call you `cd <path>` in a
Bash call of its own, since the working directory persists between calls and
`&&` is forbidden. The recipe above is then unchanged and lands at
`<path>/tmp/base`, which is gitignored there as here. The root is the state
`/ship` certifies; the main tree is the one its fixer edits while you run, and
reading it certifies nothing.
...
Read the diff via `gh pr diff $ARGUMENTS --repo haalfi/remote-store` (fall back to `pull_request_read` when `gh` is unavailable). Read every changed file **in full** for surrounding context — from the review root when a `root=` flag names one, else from the local checkout via `Read`, or `gh pr view $ARGUMENTS --json files` / `get_file_contents` for the PR-head version.
