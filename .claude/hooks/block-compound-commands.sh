...
#
# Uses Python for JSON parsing (always available) instead of jq.
...
# If Python parsing failed, allow the command through (fail-open)
...
# Check for git -C (not in allow list, triggers approval prompts)
if echo "$STRIPPED" | grep -qE '\bgit\s+-C\b'; then
  echo "BLOCKED: 'git -C' is not allowed. The working directory is already correct — just run git directly." >&2
  exit 2
fi
