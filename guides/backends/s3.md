...
From the benchmark suite ([performance guide](../performance.md#comparative-results)):

- **List 50 files**: 0.22 ms (S3 backend) — dominated by S3 API latency, not file count
- **Connection pooling**: Successive calls reuse connections; amortized cost decreases with list size
- **Under real network latency** (50–100 ms RTT): The fixed S3 API round-trip dominates; flat streams to large counts are nearly as fast as shallow lists of 50 files

For larger list benchmarks, run `hatch run bench-standard` (includes 1000 and 10000 file lists).
