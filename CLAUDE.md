...
5. **Specs are source of truth**: code vs spec conflict: code is wrong — unless the spec's claim was never enforced, which [`sdd/000-process.md` Rule 7](sdd/000-process.md#intent-attribution) qualifies, along with prose vs Dafny vs conformance generally. Backlog vs history conflict: backlog is wrong. Fix the less authoritative side.
